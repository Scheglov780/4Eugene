...
(function ($, undefined) {
    ...
    $.widget("ui.dstabs", {
        version: "1.11.3",
        delay: 300,
        extPanelClasses: "tab-panel-content-wrapper tab-pane22",
        options: {
            active: null,
            collapsible: false,
            event: "click",
            ...
            this.tabs.add(this.panels).each(function () {
                if ($.data(this, "ui-tabs-destroy")) {
                    ...
                    $(this)
                        ...
                        .removeClass(this.extPanelClasses)
                        .removeAttr("tabIndex")
        ...
        reload: function (index, event) {
            var that = this;
            this.load(index, event, true);
        },
        load: function (index, event, reload) {
            index = this._getIndex(index);
            var that = this,
            ...
            if (this.xhr && this.xhr.statusText !== "canceled") {
                ...
                this.xhr
                    .done(function (response) {
                        panel.html(response);
                        //@todo: интересно, это чё такое было и зачем?
                        //that._trigger("load", this, {tab: tab.find(".ui-tabs-anchor")[0], panel: panel[0]});
                    })
                    .always(function (jqXHR, status) {
                        ...
                        that._trigger('afterLoad',this, {tab: tab, panel: panel});
                        if (reload) {
                            that._trigger('afterReload',this, {tab: tab, panel: panel});
                        }
                        if (jqXHR === that.xhr) {
                            delete that.xhr;
                        }
...
// DEPRECATED
    if ($.uiBackCompat !== false) {
        ...
        $.ui.dstabs.prototype._ui = function (tab, panel) {
            return {
                tab: tab,
                panel: panel,
        ...
        $.widget("ui.dstabs", $.ui.dstabs, {
            _create: function () {
                var options = this.options;
                if (options.active === null && (typeof options.selected !== 'undefined')) {
