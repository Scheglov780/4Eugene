...
function checkOrder() {
    /*    var uid = $("#manager_id").val();
     $.get('/cabinet/main/checkOrder/uid/' + uid, function (data) {
     if (data) {
     var html = '<div id="order-notice">' +
     '<a href="/cabinet/orders/view" id="notice' + data + '" onclick="openOrder(' + data + ', ' + uid + ');return false;">У вас новый заказ!</a></div>';
     $("body").append(html);
     }
     else {
...
function refund(pid) {
    if (confirm('Вы уверены что хотите отменить данный товар?')) {
        ...
        var url = '/cabinet/orders/refund/pid/' + pid + '/msg/' + text;
...
function loadCatalogFromSrc(src) {
        ...
        var url = '/cabinet/menu/loadFromSrc' //form.attr('action');
...
function copyCatalogTreeToMenu(id) {
    //var form = $('#form-menu');
        ...
        var url = '/cabinet/source/copyCatalogTreeToMenu' //form.attr('action');
...
function updateMetaAndHFURL(totalCount) {
    if (confirm("Все старые значения meta и HFURL будут удалены. Продолжить?")) {
        ...
        var url = '/cabinet/menu/updateHFURL';
        var offset = 0;
        var count = 10;
        res = false;
        setTimeout(function () {
...
function updateBrandMeta() {
    if (confirm("Все старые значения meta будут удалены. Продолжить?")) {
        var url = '/cabinet/brands/updateMeta';
        $.get(url, function (data) {
            if (data==='OK') {
...
function deleteMainMenu(id) {
    if (confirm('Удалить категорию и все её дочерние категории?')) {
        var url = '/cabinet/menu/deleteex/?id=' + id;
...
function createMainMenu(id) {
    if (confirm('Добавить новую дочернюю категорию в текущую?')) {
        var url = '/cabinet/menu/createex/?id=' + id;
        $.getJSON(url, function (data) {
            getContent('/ru/cabinet/menu/update/id/' + data.id, 'Категория №' + data.id,false);
            return false;
        });
    }
    return false;
...
function clearMenuCache() {
    var postform = null;
    var url = '/ru/cabinet/cache/clear/all/2';
//    if(typeof url == "undefined"){
// url = $(sendButton).parents('form').attr('action');
// postform = $(sendButton).parents('form').serialize();
// }
...
function UpdateParamFromProxy(id) {
    var url = '/cabinet/config/updateparamfromproxy/param/' + id;
    // alert(url);
    $.get(url, function (data) {
        alert(data);
...
function cmsHistoryRestore(id) {
    if (confirm("Восстановить контент из истории изменений?")) {
        //var form = $('#form-menu');
        var postdata = 'id=' + id;
        url = '/cabinet/cmsHistory/restore';
        $.post(url, postdata, function (data) {
            alert(data);
            reloadSelectedTab(event);
        }, "text");
...
function menuStorageCommand(command, name) {
    ...
    if (confirm(message)) {
        url = '/cabinet/menu/storage/command/' + command + '/name/' + name;
        $.get(url, function (data) {
            alert(data);
            reloadSelectedTab(event);
        });
...
function generateOrderCustomXlsToMail(id,fileName,uid) {
    var postdata = 'email=' + uid;
        url = '/cabinet/orders/GenerateCustomXls?id='+id+'&fileName='+fileName;
        $.post(url, postdata, function (data) {
            alert(data);
        }, "text");
}
...
function parseUrlToMenuCategory(urlInputId,cidInputId,queryInputId) {
    ...
    url = '/cabinet/menu/parseUrlToMenuCategory';
    $.post(url, postdata, function (data) {
        if (data.result!=0) {
