...
function checkServerVar()
{
    ...
    if (!empty($missing)) {
        return t('yii', '$_SERVER does not have {vars}.', ['{vars}' => implode(', ', $missing)]);
    }
 ...
 */
function t($category, $message, $params = [])
{
    static $messages;
...
function getPreferredLanguage()
{
    if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) &&
      ($n = preg_match_all('/([\w\-]+)\s*(;\s*q\s*=\s*(\d*\.\d*))?/', $_SERVER['HTTP_ACCEPT_LANGUAGE'], $matches)) >
      0) {
        $languages = [];
        for ($i = 0; $i < $n; ++$i) {
