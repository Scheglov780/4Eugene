...
class Zend_Search_Lucene_Index_SegmentInfo implements Zend_Search_Lucene_Index_TermsStream_Interface
{
      ...
      $isCompound = null
        ...
        if ($docStoreOptions !== null) {
            ...
            if ($docStoreOptions['isCompound']) {
                ...
                if ($count != 0) {
                    $cfxFileSizes[$fileName] =
                      $this->_directory->fileLength($docStoreOptions['segment'] . '.cfx') - $dataOffset;
                }
        ...
        for ($count = 0; $count < $fieldsCount; $count++) {
            ...
            if ($fieldBits & 0x10) {
                ...
                $this->_norms[$count] =
                  str_repeat(chr(Zend_Search_Lucene_Search_Similarity::encodeNorm(1.0)), $docCount);
            }
    ...
    private function _load21DelFile()
    {
        ...
        if ($format == (int) 0xFFFFFFFF) {
            ...
            $delFileSize =
              $this->_directory->fileLength($this->_name . '_' . base_convert($this->_delGen, 10, 36) . '.del');
            $byteNum = 0;
    ...
    public function compoundFileLength($extension)
    {
        if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
            ...
            if (!isset($this->_sharedDocStoreOptions['fileSizes'][$filename])) {
                ...
                throw new Zend_Search_Lucene_Exception(
                  'Shared doc store compound file doesn\'t contain '
                  . $filename . ' file.'
                );
            }
        ...
        if (!isset($this->_segFileSizes[$filename])) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
              'Index compound file doesn\'t contain '
              . $filename . ' file.'
            );
        }
    ...
    public function getTermInfo(Zend_Search_Lucene_Index_Term $term)
    {
        ...
        $tisFile->seek(
          $prevTermInfo[4] /* indexPointer */ - (($tiVersion == (int) 0xFFFFFFFD) ? 24 : 20)
          /* header size*/,
          SEEK_CUR
        );
    ...
    public function openCompoundFile($extension, $shareHandler = true)
    {
        if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
            ...
            if (!isset($this->_sharedDocStoreOptions['files'][$fdxFName])) {
                require_once 'Zend/Search/Lucene/Exception.php';
                throw new Zend_Search_Lucene_Exception(
                  'Shared doc storage segment compound file doesn\'t contain '
                  . $fdxFName . ' file.'
                );
            }
            ...
            $cfxFile =
              $this->_directory->getFileObject($this->_sharedDocStoreOptions['segment'] . '.cfx', $shareHandler);
        ...
        if (!isset($this->_segFiles[$filename])) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
              'Segment compound file doesn\'t contain '
              . $filename . ' file.'
            );
        }
            ...
            case self::SM_MERGE_INFO:
                ...
                for ($count = 0; $count < $this->_docCount; $count++) {
                    if (!$this->isDeleted($count)) {
                        $this->_docMap[$count] =
                          $startId + (($mode == self::SM_MERGE_INFO) ? count($this->_docMap) : $count);
                    }
                }
                break;
    ...
    public function skipTo(Zend_Search_Lucene_Index_Term $prefix)
    {
        ...
        if ($highIndex == 0) {
            ...
            if ($prefix->field == $this->_lastTerm->field && $prefix->text == $this->_lastTerm->text) {
...
// ---------------------------------------------------------------
                    ...
                    for ($count = 0; $count < $termInfo->docFreq; $count++) {
                        ...
                        if (isset($filter[$docId])) {
                            $result[] = $shift + $docId;
                            $updatedFilterData[$docId] =
                              1; // 1 is just a some constant value, so we don't need additional var dereference here
                        }
                    }
                    ...
                    for ($count = 0; $count < $termInfo->docFreq; $count++) {
                        ...
                        if ($docDelta % 2 == 1) {
                            ...
                            if (isset($filter[$docId])) {
                                $result[$shift + $docId] = 1;
                                $updatedFilterData[$docId] =
                                  1; // 1 is just some constant value, so we don't need additional var dereference here
                            }
                        } else {
                            $docId += $docDelta / 2;
                            $freq = $frqFile->readVInt();
