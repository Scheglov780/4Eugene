...
class Zend_Search_Lucene_Document_Html extends Zend_Search_Lucene_Document
{
     ...
     * @param string  $defaultEncoding HTML encoding, is used if it's not specified using Content-type HTTP-EQUIV meta
     *                                 tag.
     */
     ...
     * @param array    $params   Array of additionall callback parameters (first non-optional parameter is a text to
     *                           transform)
     */
    ...
    protected function _highlightTextNode(DOMText $node, $wordsToHighlight, $callback, $params)
    {
        ...
        foreach ($matchedTokens as $token) {
            ...
            $success = @$highlightedWordNodeSetDomDocument->loadHTML(
              '<html><head><meta http-equiv="Content-type" content="text/html; charset=UTF-8"/></head><body>'
            ...
            if (!$success) {
                ...
                throw new Zend_Search_Lucene_Exception(
                  "Error occured while loading highlighted text fragment: '$highlightedWordNodeSetHtml'."
                );
            }
    ...
    public function highlightExtended($words, $callback, $params = [])
    {
        ...
        if (!is_callable($callback)) {
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new Zend_Search_Lucene_Exception(
              '$viewHelper parameter must be a View Helper name, View Helper object or callback.'
            );
        }
