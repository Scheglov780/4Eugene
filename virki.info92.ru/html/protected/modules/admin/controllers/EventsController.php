...
//$this->performAjaxValidation($model, "events-create-form");
    ...
    {
        if (Yii::app()->session->contains('Events_records')) {
            $model = Yii::app()->session->get('Events_records');
        } else {
            $model = Events::model()->findAll();
        }

        Yii::app()->request->sendFile(
          'events-' . date('YmdHis') . '.xls',
          $this->renderPartial(
            'excelReport',
            [
              'model' => $model,
...
//        Yii::app()->session->add('Events_records',Events::model()->findAll($criteria));
        $this->renderPartial(
          'index',
          [
            'model' => $model,
