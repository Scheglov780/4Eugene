...
?>
<!-- Post -->
<div class="post" id="news-block-<?= $data->news_id ?>">
  <h3><a href="javascript:void(0);"><?= $data->news_header ?></a></h3>
  <div class="user-block">
    <img class="img-circle img-bordered-sm"
         src="/themes/<?= Yii::app()->controller->module->id ?>/dist/img/user4-128x128.jpg" alt="Нет фото">
    <span class="username">
                          <a href="javascript:void(0);"><?= $data->news_author_name ?></a>
                          <a href="javascript:void(0);" class="pull-right btn-box-tool"><i class="fa fa-times"></i></a>
                        </span>
    <span class="description">Опубликовано - <?= Utils::pgDateToStr($data->created, 'd.m.Y H:i') ?></span>
  </div>
  <!-- /.user-block -->
  <p class="clearfix">
      <?= $data->news_body ?>
  ...
  <ul class="list-inline clearfix">
      <? if (Yii::app()->user->inRole(['superAdmin', 'topManager', 'contentManager'])
        ...
        <li><a class="btn btn-default btn-xs" title="<?= Yii::t('main', 'Изменение сообщения и его параметров') ?>"
               ...
               onclick="renderUpdateForm_news ('<?= $data->news_id ?>')"><i class="fa fa-pencil margin-r-5"></i>&nbsp;Редактировать</a>
        </li>
        <li><a class="btn btn-default btn-xs" title="<?= Yii::t('main', 'Удаление сообщения') ?>"
               data-toggle="tooltip"
               data-placement="top"
               href="javascript:void(0);"
               onclick="delete_record_news ('<?= $data->news_id ?>')"><i class="fa fa-trash margin-r-5"></i>&nbsp;Удалить</a>
        </li>
      <? } ?>
      <? if ($data->confirmation_needed && Yii::app()->user->id) {
          if (!$data->is_confirmed_by_current_user) { ?><?// !News::isConfirmed($data->news_id, Yii::app()->user->id) ?>
            <li><a class="btn btn-default btn-xs"
                   ...
                   data-toggle="tooltip"
                   data-placement="top"
                   href="<?= Yii::app()->createUrl('/' . Yii::app()->controller->module->id . '/news/confirm/',
                     [
                       'id' => $data->news_id,
                     ]
                   ) ?>"
                   onclick="newsConfirm(this,'<?= $data->news_id ?>'); return false;"><i
      ...
      <? if ((Yii::app()->user->inRole(
        ...
        $data->confirmation_needed &&
        $data->confirmators_count) { ?>
