...
?>
<!-- Post -->
<div class="post" id="adverts-block-<?= $data->news_id ?>">
  ...
  <ul class="list-inline clearfix">
      <? if (Yii::app()->user->inRole(['superAdmin', 'topManager', 'contentManager'])
        ...
        <li><a class="btn btn-default btn-xs" title="<?= Yii::t('main', 'Изменение сообщения и его параметров') ?>"
               ...
               onclick="renderUpdateForm_adverts ('<?= $data->news_id ?>')"><i class="fa fa-pencil margin-r-5"></i>&nbsp;Редактировать</a>
        </li>
        <li><a class="btn btn-default btn-xs" title="<?= Yii::t('main', 'Удаление сообщения') ?>"
               data-toggle="tooltip"
               data-placement="top"
               href="javascript:void(0);"
               onclick="delete_record_adverts ('<?= $data->news_id ?>')"><i class="fa fa-trash margin-r-5"></i>&nbsp;Удалить</a>
        </li>
      <? } ?>
