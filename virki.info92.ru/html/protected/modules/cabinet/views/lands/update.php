            ...
            <? if (isset($model) && $model->land_geo_longitude) { ?>
              ...
              <script>
                  $('#dsYandexMap-land-<?=$model->lands_id?> > iframe').on('load', function () {
                      /* if ($(ym).length>0) {
                          console.log('ym');
                          ym.behaviors.disable('scrollZoom');
                      } */
// создаём элемент <div>, который будем перемещать вместе с указателем мыши пользователя
                      $('.YandexMap').parent().find('#dsYandexMap-land-<?=$model->lands_id?>').each(
                          function () {
        ...
        $('#Lands_users_update_single-<?=$model->lands_id?>').select2(
            {
                ...
                templateSelection: function (state) {
                    if (!state.id) {
                        ...
                        return state.text.trim() +
                            '&nbsp;<a href="/<?=Yii::app(
                            )->controller->module->id?>/profile/view" title="Просмотр профиля пользователя" onclick="getContent(this,\'' +
                            'Профиль' + //state.text.trim() +
                            '\',false);return false;"><i class="fa fa-external-link fa-fw text-white"></i></a>';
                    }
                },
                escapeMarkup: function (markup) {
