...
class MOXMAN_Http_HttpClientFormData
{
    ...
    {
        $this->items[$name] = [$path, $filename ? $filename : basename($path), $mime];
...
class MOXMAN_Http_HttpClientRequest
{
    ...
    private function fputs($data, $isBody = false)
    {
        if ($this->client->getLogLevel() >= 3) {
            ...
            if ($this->client->getLogLevel() == 2 && !$isBody) {
                $this->client->log("> " . trim($data));
            }
        }
        ...
        foreach ($items as $name => $item) {
            if (is_string($item)) {
                ...
                if (!file_exists($item[0])) {
                    throw new MOXMAN_Http_HttpClientException(
                      "Could not open file: " . $item[0] . " for upload using multipart."
                    );
                }
    ...
    private function sendCurl($data)
    {
        ...
        curl_setopt_array($ch, [
          CURLOPT_HEADERFUNCTION => [$this, 'curlCallbackWriteHeader'],
          CURLOPT_WRITEFUNCTION  => [$this, 'curlCallbackWriteBody'],
    ...
    private function sendSocket($data)
    {
        ...
        if ($this->client->getLogLevel() >= 1) {
            $this->client->log(
              "HTTP: (" . strtoupper($this->method) . ") " . $this->url["path"] . ($query ? "?" . $query : "")
            );
        }

        // Setup base of request
        $this->fputs(
          strtoupper($this->method) . " " . $this->url["path"] . ($query ? "?" . $query : "") . " HTTP/1.1\n"
        );
...
class MOXMAN_Http_HttpClientResponse
{
    ...
    public function __construct($client, $req, $contentLength = 0)
    {
        ...
        if ($method == "head" ||
          $code == 204 ||
          $code == 304 ||
          ($method == "connect" && $code >= 200 && $code < 300)) {
