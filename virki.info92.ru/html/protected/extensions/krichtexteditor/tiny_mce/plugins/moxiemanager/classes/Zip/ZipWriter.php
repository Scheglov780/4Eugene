...
class MOXMAN_Zip_ZipWriter
{
    ...
    public function addDirectory($zipPath, $path = null)
    {
        if ($path) {
            ...
            foreach ($files as $file) {
                $this->addEntry($this->combine($zipPath, substr($file, strlen($path))), (object) ["file" => $file]);
            }
        } else {
    ...
    public function addFile($zipPath, $path)
    {
        if (is_dir($path)) {
            ...
            $this->addEntry($zipPath, (object) ["file" => $path]);
        }
    }
    ...
    {
        $this->addEntry($zipPath, (object) ["data" => $data]);
    }
