...
class MOXMAN_Vfs_FileSystemManager
{
            ...
            if (preg_match('/^\{([0-9]+)\}/', $path, $matches)) {
                ...
                if (!isset($this->fileSystems[$rootIdx])) {
                    throw new MOXMAN_Exception(
                      "Could not decode {" . $rootIdx . "} with a root path. Index not found."
                    );
                }
            ...
            foreach ($this->fileSystems as $fileSystem) {
                if (!$scheme || $fileSystem->getScheme() === $scheme) {
                    ...
                    if (strpos($path, $rootPrefix) === 0) {
                        $filePath =
                          MOXMAN_Util_PathUtils::combine(
                            $fileSystem->getRootPath(),
                            substr($path, strlen($rootPrefix))
                          );
    ...
    public function getLocalTempPath(MOXMAN_Vfs_IFile $file)
    {
        ...
        $tempDir =
          MOXMAN_Util_PathUtils::combine(MOXMAN_Util_PathUtils::getTempDir($this->config), "moxman_blob_cache");
        $tempFilePath =
          MOXMAN_Util_PathUtils::combine(
            $tempDir,
            md5($file->getPath() . $file->getLastModified()) .
            "." .
            MOXMAN_Util_PathUtils::getExtension($file->getName())
          );
