...
class MOXMAN_Util_PathUtils
{
    ...
    public static function combine($path1, $path2)
    {
        ...
        return preg_replace('/\\/$/', '', self::toUnixPath($path1)) .
          '/' .
          preg_replace('/^\\/|\\/$/', '', self::toUnixPath($path2));
    }
    ...
    public static function getSiteRoot()
    {
        ...
        if (isset($_SERVER["SCRIPT_NAME"]) && isset($_SERVER["SCRIPT_FILENAME"])) {
            $path =
              str_replace(self::toUnixPath($_SERVER["SCRIPT_NAME"]), "", self::toUnixPath($_SERVER["SCRIPT_FILENAME"]));
        ...
        if (isset($_SERVER["SCRIPT_NAME"]) && isset($_SERVER["PATH_TRANSLATED"])) {
            $path =
              str_replace(self::toUnixPath($_SERVER["SCRIPT_NAME"]), "", self::toUnixPath($_SERVER["PATH_TRANSLATED"]));
    ...
    public static function verifyPath($path, $isLocalPath = false, $fileType = null)
    {
        ...
        if ($isLocalPath) {
                ...
                if ($version < 7) {
                    if (strpos($path, ';') !== false) {
                        throw new MOXMAN_Exception(
                          "IIS 6 doesn't support semicolon in paths for security reasons.",
                    ...
                    if (preg_match('/\.[^\/]+\//', $path) || ($fileType == "dir" && strpos($path, '.') !== false)) {
                        throw new MOXMAN_Exception(
                          "IIS 6 don't support dots in directory names for security reasons.",
                          MOXMAN_Exception::INVALID_FILE_NAME
                        );
                    }
                }
            ...
            if (preg_match('/.(php|inc|php\d+|phtml|php[st])\.[^\/]+/', $path)) {
                throw new MOXMAN_Exception(
                  "Double extensions is not allowed for security reasons.",
                  MOXMAN_Exception::INVALID_FILE_NAME
                );
            }
        }
