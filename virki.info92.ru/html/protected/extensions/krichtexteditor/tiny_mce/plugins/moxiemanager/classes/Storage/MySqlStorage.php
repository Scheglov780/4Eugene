        ...
        switch ($this->type) {
            case self::TYPE_GLOBAL:
                if ($pdo->i(
                    ...
                    $pdo->q(
                      "UPDATE moxman_property SET pr_value = :value, pr_modificationdate = NOW() WHERE pr_user IS NULL AND pr_group IS NULL AND pr_name = :name",
                      [
                        "name"  => $name,
                        "value" => $value,
                      ]
                    ...
                    $pdo->q(
                      "INSERT INTO moxman_property(pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:name, :value, NOW(), NOW())",
                      [
                        "name"  => $name,
                        "value" => $value,
                      ]
            ...
            case self::TYPE_GROUP:
                if ($pdo->i(
                    ...
                    $pdo->q(
                      "UPDATE moxman_property SET pr_value = :value, pr_modificationdate = NOW() WHERE pr_group = :grname AND pr_name = :name",
                      [
                        "grname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
                    ...
                    $pdo->q(
                      "INSERT INTO moxman_property(pr_group, pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:grname, :name, :value, NOW(), NOW())",
                      [
                        "grname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
            ...
            case self::TYPE_USER:
                if ($pdo->i(
                    ...
                    $pdo->q(
                      "UPDATE moxman_property SET pr_value = :value, pr_modificationdate = NOW() WHERE pr_user = :usname AND pr_name = :name",
                      [
                        "usname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
                    ...
                    $pdo->q(
                      "INSERT INTO moxman_property(pr_user, pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:usname, :name, :value, NOW(), NOW())",
                      [
                        "usname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
