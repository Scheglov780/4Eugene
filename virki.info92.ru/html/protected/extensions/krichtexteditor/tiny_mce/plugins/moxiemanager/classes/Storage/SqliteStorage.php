 ...
 */
class MOXMAN_Storage_SqliteStorage implements MOXMAN_Storage_IStorage
{
    /** @ignore */
    ...
    public function get($name, $default = null)
    {
        ...
        switch ($this->type) {
            case self::TYPE_GLOBAL:
                $statement =
                  $pdo->q(
                    "SELECT pr_name, pr_value FROM moxman_property WHERE pr_user IS NULL AND pr_group IS NULL AND pr_name = ?",
                    $name
                  );
                break;
    ...
    public function getAll()
    {
        ...
        switch ($this->type) {
            case self::TYPE_GLOBAL:
                $statement =
                  $pdo->q("SELECT pr_name, pr_value FROM moxman_property WHERE pr_user IS NULL AND pr_group IS NULL");
                break;
    ...
    public function put($name, $value)
    {
        ...
        switch ($this->type) {
            case self::TYPE_GLOBAL:
                if ($pdo->i(
                    'SELECT COUNT(pr_id) FROM moxman_property WHERE pr_user IS NULL AND pr_group IS NULL AND pr_name = ?',
                    $name
                  ) > 0) {
                    $pdo->q(
                      "UPDATE moxman_property SET pr_value = :value, pr_modificationdate = DATETIME('NOW') WHERE pr_user IS NULL AND pr_group IS NULL AND pr_name = :name",
                      [
                        "name"  => $name,
                        "value" => $value,
                      ]
                    ...
                    $pdo->q(
                      "INSERT INTO moxman_property(pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:name, :value, DATETIME('NOW'), DATETIME('NOW'))",
                      [
                        "name"  => $name,
                        "value" => $value,
                      ]
            ...
            case self::TYPE_GROUP:
                if ($pdo->i(
                    'SELECT COUNT(pr_id) FROM moxman_property WHERE pr_group = ? AND pr_name = ?',
                    $this->name,
                    $name
                  ) > 0) {
                    $pdo->q(
                      "UPDATE moxman_property SET pr_value = :value, pr_modificationdate = DATETIME('NOW') WHERE pr_group = :grname AND pr_name = :name",
                      [
                        "grname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
                    ...
                    $pdo->q(
                      "INSERT INTO moxman_property(pr_group, pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:grname, :name, :value, DATETIME('NOW'), DATETIME('NOW'))",
                      [
                        "grname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
            ...
            case self::TYPE_USER:
                if ($pdo->i(
                    'SELECT COUNT(pr_id) FROM moxman_property WHERE pr_user = ? AND pr_name = ?',
                    $this->name,
                    $name
                  ) > 0) {
                    $pdo->q(
                      "UPDATE moxman_property SET pr_value = :value, pr_modificationdate = DATETIME('NOW') WHERE pr_user = :usname AND pr_name = :name",
                      [
                        "usname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
                    ...
                    $pdo->q(
                      "INSERT INTO moxman_property(pr_user, pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:usname, :name, :value, DATETIME('NOW'), DATETIME('NOW'))",
                      [
                        "usname" => $this->name,
                        "name"   => $name,
                        "value"  => $value,
    ...
    public function remove($name)
    {
        ...
        switch ($this->type) {
            case self::TYPE_GLOBAL:
                $pdo->q(
                  "DELETE FROM moxman_property WHERE pr_user IS NULL and pr_group IS NULL AND pr_name = ?",
                  $name
                );
                break;
