...
class MOXMAN_Core_UploadHandler implements MOXMAN_Http_IHandler
{
    ...
    public function processRequest(MOXMAN_Http_Context $httpContext)
    {
        ...
        try {
            ...
            if (!$file->canWrite()) {
                throw new MOXMAN_Exception(
                  "No write access to path: " . $file->getPublicPath(),
            ...
            if ($total > $maxSizeBytes) {
                throw new MOXMAN_Exception(
                  "File size to large: " . $file->getPublicPath(),
                  MOXMAN_Exception::FILE_SIZE_TO_LARGE
                );
            }
            ...
            if ($loaded == 0) {
                ...
                if ($file->exists()) {
                    if (!$config->get("upload.overwrite") && !$request->get("overwrite")) {
                        throw new MOXMAN_Exception(
                          "Target file exists: " . $file->getPublicPath(),
            ...
            if ($total == -1 || $loaded + $blobSize == $total) {
                ...
                if ($config->get("upload.autoresize") &&
                  preg_match('/gif|jpe?g|png/i', MOXMAN_Util_PathUtils::getExtension($tempFilePath)) === 1) {
