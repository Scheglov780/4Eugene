...
class MOXMAN_Core_Plugin implements MOXMAN_IPlugin, MOXMAN_ICommandHandler, MOXMAN_Http_IHandler
{
    ...
    /** @ignore */
    private function fireFileAction($action, $file, $data = [])
    {
    ...
    public function createThumbnail(MOXMAN_Vfs_IFile $file, $localTempFile = null)
    {
        ...
        if ($file instanceof MOXMAN_Vfs_Local_File) {
            if ($config->get('thumbnail.use_exif') &&
              function_exists("exif_thumbnail") &&
              preg_match('/jpe?g/i', MOXMAN_Util_PathUtils::getExtension($file->getName()))) {
                $imageType = null;
                $width = 0;
                $height = 0;
    ...
    public static function fileToJson($file, $meta = false)
    {
        ...
        if ($meta) {
            ...
            if ($file instanceof MOXMAN_Vfs_Local_File && MOXMAN_Media_ImageAlter::canEdit($file)) {
                $thumbnailFolderPath =
                  MOXMAN_Util_PathUtils::combine($file->getParent(), $config->get('thumbnail.folder'));
                $thumbnailFile =
                  MOXMAN::getFile($thumbnailFolderPath, $config->get('thumbnail.prefix') . $file->getName());
