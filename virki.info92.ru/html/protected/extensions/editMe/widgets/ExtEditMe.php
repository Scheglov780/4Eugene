...
class ExtEditMe extends CInputWidget
{
    ...
    protected function _ckeGenerateConfig()
    {
        ...
        */
        //$ckeConfig['removeButtons'] = (empty($this -> removeButtons)) ? null : $this -> removeButtons;
        if (isset($this->toolbar) && is_array($this->toolbar) && isset($this->toolbar['removeButtons'])) {
            $ckeConfig['removeButtons'] = $this->toolbar['removeButtons'];
        }

        // Generate "forcePasteAsPlainText" config value
        $ckeConfig['forcePasteAsPlainText'] =
          (isset($toolbarItems['Paste']) || isset($toolbarItems['PasteFromWord'])) ? false : true;
        // Generate "removeDialogTabs" config value
        $ckeConfig['removeDialogTabs'] =
          ($this->advancedTabs === false) ?
            'imagebutton:advanced;creatediv:advanced;editdiv:advanced;link:advanced;image:advanced;table:advanced;tableProperties:advanced;flash:advanced;iframe:advanced;' :
            '';
        // Generate "contentsCss" config value
        $ckeConfig['contentsCss'] = array_merge([self::$_ckeAssetUrl . '/contents.css'], (array) $this->contentsCss);
            ...
            if (empty(self::$_languages)) {
                ...
                if (empty($languages)) {
                    throw new CException(
                      'Cannot access CKEditor language folder "' . $this->_ckeExtensionPath . '/lang".'
                    );
                }
    ...
    public function init()
    {
        ...
        if (empty(self::$_ckeAssetUrl)) {
            ...
            $ckeAssetPath =
              str_replace(Yii::app()->assetManager->baseUrl, Yii::app()->assetManager->basePath, self::$_ckeAssetUrl);
            if (@is_file($ckeAssetPath . '/editMe.' . md5(self::$_ckeAssetUrl) . '.min.js') === false) {
                ...
                @file_put_contents(
                  $ckeAssetPath . '/editMe.' . md5(self::$_ckeAssetUrl) . '.min.js',
                  implode("\n", $scriptContents),
                  LOCK_EX
                );
            }
    ...
    public function run()
    {
        ...
        Yii::app()->clientScript->registerScriptFile(
          self::$_ckeAssetUrl . '/editMe.' . md5(self::$_ckeAssetUrl) . '.min.js'
        ...
        if (empty($this->inlineId)) {
            ...
            Yii::app()->clientScript->registerScript(
              'editMe_' . $this->htmlOptions['id'],
              'jQuery(' . $jqSelector . ').ckeditor(' . $ckeConfig . ');',
            ...
            Yii::app()->clientScript->registerScript(
              'editMe_inline_' . $this->inlineId,
              'jQuery(' .
              $jqSelector .
              ').attr("contenteditable",true);CKEDITOR.disableAutoInline=true;CKEDITOR.inline(' .
              $selector .
              ',' .
              $ckeConfig .
              ');',
