...
class ModelCode extends CCodeModel
{
    ...
    protected function generateClassName($tableName)
    {
        if ($this->tableName === $tableName ||
          ($pos = strrpos($this->tableName, '.')) !== false &&
          substr($this->tableName, $pos + 1) === $tableName) {
            return $this->modelClass;
        }

        $tableName = $this->removePrefix($tableName, false);
        if (($pos = strpos($tableName, '.')) !==
          false) // remove schema part (e.g. remove 'public2.' from 'public2.post')
        {
        ...
        foreach (Yii::app()->{$this->connectionId}->schema->getTables($schemaName) as $table) {
            ...
            if ($this->isRelationTable($table)) {
                ...
                $relations[$className0][$relationName] =
                  "array(self::MANY_MANY, '$className1', '$unprefixedTableName($pks[0], $pks[1])')";
                ...
                $relations[$className1][$relationName] =
                  "array(self::MANY_MANY, '$className0', '$unprefixedTableName($pks[1], $pks[0])')";
            } else {
    ...
    protected function isRelationTable($table)
    {
        ...
        return ($count === 2
          ...
          &&
          $table->foreignKeys[$pk[0]][0] !==
          $table->foreignKeys[$pk[1]][0]); // and the foreign keys point different tables
    }
        ...
        foreach ($table->columns as $column) {
            if ($this->commentsAsLabels && $column->comment) {
                ...
                $label =
                  ucwords(
                    trim(
                      strtolower(str_replace(['-', '_'], ' ', preg_replace('/(?<![A-Z])[A-Z]/', ' \0', $column->name)))
                    )
                  );
    ...
    public function generateRules($table)
    {
        ...
        if ($required !== []) {
            $rules[] = "array('" . implode(', ', $required) . "', 'required', 'on' => 'update')";
        }
        if ($integers !== []) {
    ...
    public function prepare()
    {
        ...
        foreach ($tables as $table) {
            ...
            $this->files[] = new CCodeFile(
              Yii::getPathOfAlias($this->modelPath) . '/custom/custom' . $className . '.php',
              $this->render($templatePath . '/customModel.php', $params)
            );
        }
    }
    ...
    public function validateConnectionId($attribute, $params)
    {
        if (Yii::app()->hasComponent($this->connectionId) === false ||
          !(Yii::app()->getComponent($this->connectionId) instanceof CDbConnection)) {
