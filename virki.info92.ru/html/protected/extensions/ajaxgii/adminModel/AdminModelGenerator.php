...
class AdminModelGenerator extends CCodeGenerator
{
    public $codeModel = 'application.extensions.ajaxgii.adminModel.ModelCode';
    ...
    public function actionGetTableNames($db)
    {
        if (Yii::app()->getRequest()->getIsAjaxRequest()) {
            $all = [];
            if (!empty($db) &&
              Yii::app()->hasComponent($db) !== false &&
              (Yii::app()->getComponent($db) instanceof CDbConnection)) {
