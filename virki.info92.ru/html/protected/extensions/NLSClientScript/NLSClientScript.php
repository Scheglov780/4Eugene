...
class NLSClientScript extends \CClientScript
{
     ...
     * - it doesn't consider @imports inside css files, only counts the top-level files and compares the sum with
     * $mergeAbove
     **/
    public $mergeAbove = 0;
    /**
     ...
     *                              you may define the url of the DOCROOT on the server (defaults to a composed value
     *                              from the $_SERVER members)
     **/
        ...
        if ($this->mergeJs && !empty($this->scriptFiles[$pos]) && count($this->scriptFiles[$pos]) > $this->mergeAbove) {
            ...
            if ($this->forceMergeJs || !file_exists($path)) {
                ...
                if ($this->compressMergedJs) {
                    $merged = \JShrink\Minifier::minify($merged);
    ...
    protected function addAppVersion($url)
    {
        if (!empty($this->appVersion) && !NLSUtils::isAbsoluteUrl($url)) {
            $url = NLSUtils::addUrlParams($url, ['nlsver' => $this->appVersion]);
        }
        return $url;
    }
    ...
    public function init()
    {
        ...
        if (is_numeric($this->nlsScriptPosition)) //-> we need jquery
        {
            $this->registerCoreScript('jquery');
        }
// Alexy - need for migrating jquery
        /* if (YII_DEBUG) {
             $this->coreScripts['jquery']['js'][]='jquery-migrate.js';
         } */
        ...
        $this->downloader = new NLSDownloader([
        ...
        //setup css merger
        $this->cssMerger = new NLSCssMerge([
    ...
    public function isMetaTagExists($id)
    {
        return isset($this->metaTags[$id]);
    }
...
//Alexys custom additional function
