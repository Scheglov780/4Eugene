...
class Hybrid_Endpoint
{
        ...
        if (!$this->initDone) {
            ...
            try {
                ...
                throw new Hybrid_Exception(
                  "Endpoint: Error while trying to init Hybrid_Auth: " . $e->getMessage(),
                  $e->getCode(),
                  $e
                );
            }
    ...
    protected function processAuthStart()
    {
        ...
        if (!Hybrid_Auth::storage()->get("hauth_session.$provider_id.hauth_endpoint")) {
            Hybrid_Logger::error(
              "Endpoint: hauth_endpoint parameter is not defined on hauth_start, halt login process!"
            );
    ...
    protected function processOpenidRealm()
    {
        $output = str_replace(
          "{X_XRDS_LOCATION}",
          htmlentities(Hybrid_Auth::getCurrentUrl(false), ENT_QUOTES, 'UTF-8')
          . "?get=openid_xrds&v="
          . Hybrid_Auth::$version,
