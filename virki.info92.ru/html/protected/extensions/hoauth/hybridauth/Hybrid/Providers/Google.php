...
class Hybrid_Providers_Google extends Hybrid_Provider_Model_OAuth2
{
        ...
        if (strpos($this->scope, '/m8/feeds/') !== false) {
            ...
            if (isset($response->feed->entry)) {
                foreach ($response->feed->entry as $idx => $entry) {
                    ...
                    $uc->email =
                      isset($entry->{'gd$email'}[0]->address) ? (string) $entry->{'gd$email'}[0]->address : '';
                        ...
                        if (is_array($entry->link)) {
                            foreach ($entry->link as $l) {
                                if (property_exists($l, 'gd$etag') && $l->type == "image/*") {
                                    $uc->photoURL =
                                      $this->addUrlParam($l->href, ['access_token' => $this->api->access_token]);
                                } else {
                                    if ($l->type == "self") {
                                        $uc->profileURL =
                                          $this->addUrlParam($l->href, ['access_token' => $this->api->access_token]);
    ...
    function getUserProfile()
    {
        ...
        $this->user->profile->emailVerified =
          (property_exists($response, 'email_verified')) ?
            ($response->email_verified === true || $response->email_verified === 1 ? $response->email : "") : "";
