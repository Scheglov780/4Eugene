...
class Hybrid_Providers_Yahoo extends Hybrid_Provider_Model_OAuth2
{
    ...
    function getCurrentUserId()
    {
        ...
        $response = $this->api->get("me/guid", [
          "format" => "json",
        ]);

        if (!isset($response->guid->value)) {
            throw new Exception(
              "User id request failed! {$this->providerId} returned an invalid response: " .
              Hybrid_Logger::dumpData($response)
            );
        }
    ...
    function getUserContacts()
    {
        ...
        $response = $this->api->get("user/{$userId}/contacts", [
          "format" => "json",
          "count"  => "max",
        ]);
        ...
        if ($this->api->http_code != 200) {
            throw new Exception(
              "User contacts request failed! {$this->providerId} returned an error: " . $this->errorMessageByStatus()
            );
        }
    ...
    function getUserProfile()
    {
        ...
        $response = $this->api->get("user/{$userId}/profile", [
          "format" => "json",
        ]);
