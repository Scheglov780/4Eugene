...
class Hybrid_Providers_Facebook extends Hybrid_Provider_Model
{
    ...
    public $api;
    /**
     * Default permissions, and a lot of them. You can change them from the configuration by setting the scope to what
     * you want/need. For a complete list see: https://developers.facebook.com/docs/facebook-login/permissions
    ...
    function getUserActivity($stream = 'timeline')
    {
        try {
            ...
            throw new Hybrid_Exception(
              "User activity stream request failed! {$this->providerId} returned an error: {$e->getMessage()}", 0, $e
            );
        }
    ...
    function getUserContacts()
    {
        ...
        while ($pagedList) {
            try {
                ...
                throw new Hybrid_Exception(
                  "User contacts request failed! {$this->providerId} returned an error {$e->getMessage()}", 0, $e
                );
            }
        ...
        foreach ($returnedContacts as $item) {
            ...
            $uc->profileURL =
              (array_key_exists("link", $item)) ? $item["link"] :
    ...
    function getUserPages($writableonly = false)
    {
        ...
        try {
            ...
            throw new Exception(
              "Cannot retrieve user pages! {$this->providerId} returned an error: {$e->getMessage()}",
              0,
              $e
            );
        }
    ...
    function getUserProfile()
    {
        try {
            ...
            throw new Exception(
              "User profile request failed! {$this->providerId} returned an error: {$e->getMessage()}", 6, $e
            );
        }
        ...
        $this->user->profile->region =
          (array_key_exists("location", $data) && array_key_exists("name", $data['location'])) ?
            $data['location']["name"] : "";
    ...
    function initialize()
    {
        ...
        $this->api = new FacebookSDK([
          ...
          'default_graph_version' => !empty($this->config['default_graph_version']) ?
            $this->config['default_graph_version'] : 'v2.12',
    ...
    function loginFinish()
    {
        ...
        if (!isset($accessToken)) {
            if ($helper->getError()) {
                throw new Hybrid_Exception(
                  sprintf(
                    "Could not authorize user, reason: %s (%d)",
    ...
    function setUserStatus($status, $pageid = null)
    {
        ...
        try {
            ...
            throw new Exception(
              "Update user status failed! {$this->providerId} returned an error {$e->getMessage()}",
              0,
              $e
            );
        }
