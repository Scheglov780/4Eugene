...
class Hybrid_Providers_Twitter extends Hybrid_Provider_Model_OAuth1
{
    ...
    function getUserActivity($stream)
    {
        ...
        if ($this->api->http_code != 200) {
            throw new Exception(
              "User activity stream request failed! {$this->providerId} returned an error. " .
        ...
        foreach ($response as $item) {
            ...
            $ua->user->profileURL =
              (property_exists($item->user, 'screen_name')) ? ("http://twitter.com/" . $item->user->screen_name) : "";
            $ua->user->photoURL =
              (property_exists($item->user, 'profile_image_url')) ? $item->user->profile_image_url : "";
    ...
    function getUserContacts()
    {
        ...
        if ($this->api->http_code != 200) {
            throw new Exception(
              "User contacts request failed! {$this->providerId} returned an error. " .
              $this->errorMessageByStatus($this->api->http_code)
            );
        }
        ...
        foreach ($contactsids as $chunk) {
            ...
            if ($response && count($response)) {
                foreach ($response as $item) {
                    ...
                    $uc->profileURL =
                      (property_exists($item, 'screen_name')) ? ("http://twitter.com/" . $item->screen_name) : "";
    ...
    function getUserProfile()
    {
        ...
        if ($this->api->http_code != 200) {
            throw new Exception(
              "User profile request failed! {$this->providerId} returned an error. " .
        ...
        if (!is_object($response) || !isset($response->id)) {
            throw new Exception(
              ...
              Hybrid_Logger::dumpData($response), 6
            );
        }
        ...
        $this->user->profile->photoURL =
          (property_exists($response, 'profile_image_url')) ?
            (str_replace('_normal', '', $response->profile_image_url)) : "";
        $this->user->profile->profileURL =
          (property_exists($response, 'screen_name')) ? ("http://twitter.com/" . $response->screen_name) : "";
    ...
    function getUserStatus($tweetid)
    {
        ...
        if ($this->api->http_code != 200 || !isset($info->id)) {
            throw new Exception(
              "Cannot retrieve user status! {$this->providerId} returned an error. " .
              $this->errorMessageByStatus($this->api->http_code)
            );
        }
    ...
    function loginBegin()
    {
        ...
        if (isset($_REQUEST['reverse_auth']) && ($_REQUEST['reverse_auth'] == 'yes')) {
            $stage1 =
              $this->api->signedRequest($this->api->request_token_url, 'POST', ['x_auth_mode' => 'reverse_auth']);
            if ($this->api->http_code != 200) {
                throw new Exception(
                  "Authentication failed! {$this->providerId} returned an error. " .
                  $this->errorMessageByStatus($this->api->http_code), 5
                );
            }
            $responseObj =
              ['x_reverse_auth_parameters' => $stage1, 'x_reverse_auth_target' => $this->config["keys"]["key"]];
        ...
        if ((isset($this->config['force_login']) && $this->config['force_login']) ||
          (isset($this->config['force']) && $this->config['force'] === true)) {
    ...
    function setUserStatus($status)
    {
        ...
        if ($this->api->http_code != 200) {
            throw new Exception(
              "Update user status failed! {$this->providerId} returned an error. " .
              $this->errorMessageByStatus($this->api->http_code)
            );
        }
