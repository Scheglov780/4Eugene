...
class Hybrid_Auth
{
    ...
    public static function authenticate($providerId, $params = null)
    {
        ...
        if (!Hybrid_Auth::storage()->get("hauth_session.$providerId.is_logged_in")) {
            ...
            Hybrid_Logger::info(
              "Hybrid_Auth::authenticate( $providerId ), User not connected to the provider. Try to authenticate.."
            );
            ...
            Hybrid_Logger::info(
              "Hybrid_Auth::authenticate( $providerId ), User is already connected to this provider. Return the adapter instance."
            );
    ...
    public static function initialize($config)
    {
        ...
        Hybrid_Logger::debug(
          "Hybrid_Auth initialize. dump current session: ",
        ...
        if (Hybrid_Error::hasError()) {
            ...
            Hybrid_Logger::error(
              "Hybrid_Auth initialize: A stored Error found, Throw an new Exception and delete it from the store: Error#$c, '$m'"
            );
    ...
    public static function redirect($url, $mode = "PHP")
    {
        ...
        if ((PHP_VERSION_ID >= 50400 && PHP_SESSION_ACTIVE === session_status()) ||
          (PHP_VERSION_ID < 50400 && isset($_SESSION) && session_id())) {
            session_write_close();
        }
    ...
    public static function setup($providerId, $params = null)
    {
        ...
        if (!$params) {
            ...
            Hybrid_Logger::debug(
              "Hybrid_Auth::setup( $providerId ), no params given. Trying to get the stored for this provider.",
              $params
            );
        }
        ...
        if (!$params) {
            $params = [];
            Hybrid_Logger::info(
              "Hybrid_Auth::setup( $providerId ), no stored params found for this provider. Initialize a new one for new session"
            );
        }
        ...
        if (is_array($params) && !isset($params["hauth_return_to"])) {
            ...
            Hybrid_Logger::debug(
              "Hybrid_Auth::setup( $providerId ). HybridAuth Callback URL set to: ",
     ...
     * Users sessions are stored using HybridAuth storage system ( HybridAuth 2.0 handle PHP Session only) and can be
     * accessed directly by Hybrid_Auth::storage()->get($key) to retrieves the data for the given key, or calling
