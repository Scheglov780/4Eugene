...
class Hybrid_Provider_Model_OAuth2 extends Hybrid_Provider_Model
{
    ...
    function loginFinish()
    {
        ...
        try {
            ...
            throw new Exception(
              "User profile request failed! {$this->providerId} returned an error: " . $e->getMessage(), 6
            );
        }
    ...
    function refreshToken()
    {
        if ($this->api->access_token) {
            ...
            if (
              $this->api->access_token_expires_at &&
              $this->api->access_token_expires_at <= time()
                ...
                if ($this->api->refresh_token) {
                    ...
                    if (empty($response->access_token)) {
                        // Set the user as disconnected and throw an exception.
                        $this->setUserUnconnected();

                        throw new Exception(
                          "The Authorization Service has return an invalid response while requesting a new access token. " .
