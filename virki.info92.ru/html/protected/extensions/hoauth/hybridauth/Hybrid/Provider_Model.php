...
abstract class Hybrid_Provider_Model
{
    ...
    function __construct($providerId, $config, $params = null)
    {
        ...
        Hybrid_Logger::debug(
          "Hybrid_Provider_Model::__construct( $providerId ) initialized. dump current adapter instance: ",
          serialize($this)
        );
    }
    ...
    public function login()
    {
        ...
        if (!isset($this->params["login_start"])) {
            $this->params["login_start"] =
              $base_url .
              (strpos($base_url, '?') ? '&' : '?') .
              "hauth.start={$this->providerId}&hauth.time={$this->params["hauth_time"]}";
        }
        ...
        if (!isset($this->params["login_done"])) {
            $this->params["login_done"] =
              $base_url . (strpos($base_url, '?') ? '&' : '?') . "hauth.done={$this->providerId}";
        }
        ...
        if (isset($this->params["hauth_return_to"])) {
            Hybrid_Auth::storage()->set(
              "hauth_session.{$this->providerId}.hauth_return_to",
              $this->params["hauth_return_to"]
            );
        }
        if (isset($this->params["login_done"])) {
            Hybrid_Auth::storage()->set(
              "hauth_session.{$this->providerId}.hauth_endpoint",
              $this->params["login_done"]
            );
        }
        ...
        // Move on.
        Hybrid_Logger::debug(
          "Hybrid_Provider_Model::login( {$this->providerId} ), redirect the user to login_start URL."
        );
