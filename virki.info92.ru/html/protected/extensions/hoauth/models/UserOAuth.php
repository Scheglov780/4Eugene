...
class UserOAuth extends CActiveRecord
{
    ...
    protected function updateDb($model)
    {
        ...
        try {
            ...
            ob_start();
            ?>
          ALTER TABLE <?php echo '`' .
              $model->tableName() .
              '`'; ?> CHANGE `name` `provider` VARCHAR( 45 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL ,
        ...
        try {
          ...
          ALTER TABLE <?php echo '`' .
              $model->tableName() .
              '`'; ?> ADD `profile_cache` TEXT NOT NULL AFTER `identifier`
            <?php
            $updates[] = ob_get_clean();
        }
    ...
    public function authenticate($provider)
    {
        if (empty($this->provider)) {
            try {
                ...
                $oAuth = self::model()->findByPk(['provider' => $this->provider, 'identifier' => $this->identifier]);
                if ($oAuth) {
     ...
     * We implement this method, because in older version of hoauth was no profile cache. So we need to fill db with
     * caches The second reason is camelCase
     */
    ...
    protected static function createDbTable()
    {
        ...
        $sql = strtr($sql, ['{{user_oauth}}' => Yii::app()->db->tablePrefix . 'user_oauth']);
        Yii::app()->db->createCommand($sql)->execute();
    }
    ...
    public static function getConfigPath()
    {
        ...
        if (empty($config)) {
            ...
            if (strpos($config, DIRECTORY_SEPARATOR) === false) {
