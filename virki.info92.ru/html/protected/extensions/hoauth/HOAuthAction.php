 ...
 * HOAuthAction requires, that your user model implements findByEmail() method, that should return user model by its
 * email. Avaible social networks: OpenID, Google, Facebook, Twitter, Yahoo, MySpace, Windows Live, LinkedIn,
 * Foursquare, AOL Additional social networks can be found at: {@link http://hybridauth.sourceforge.net/download.html}
 * Social Auth widget:
...
{
    /**
     * @var array $avaibleAtts Hybridauth attributes that support by this script (this a list of all available
     *      attributes in HybridAuth 2.0.11) + additional attributes (see $attributes property)
     */
    protected $_avaibleAtts = [
      'identifier',
      'profileURL',
      'webSiteURL',
      'photoURL',
    ...
    /**
     * @var array $attributes attributes synchronization array (user model attribute => oauth attribute). List of
     *      available profile attributes you can see at
     *      {@link http://hybridauth.sourceforge.net/userguide/Profile_Data_User_Profile.html "HybridAuth's Documentation"}. Additional attributes: birthDate - The full date of birthday, eg. 1991-09-03 genderShort - short representation of gender, eg. 'm', 'f' You can also set attributes, that you need to save in model too, eg.:
     *    'attributes' => array(
     *      'is_active' => 1,
    ...
    /**
     * @var boolean $enabled defines whether the ouath functionality is active. Useful for example for CMS, where user
     *      can enable or disable oauth functionality in control panel
     */
    public $enabled = true;
    /**
    ...
    protected function checkUserData($user)
    {
        ...
        if (!$form->validateUser()) {
            ...
            $this->controller->render('hoauth.views.form', [
              'form' => $form,
            ]);
            Yii::app()->end();
        }
    ...
    protected function getAccessCode()
    {
        ...
        if (!Yii::app()->user->isGuest) {
            ...
            [$user, $isNewUser] = $this->prepareGuestUser();
            ...
            switch ($accessCode) {
                case 1:
                    if (!$oAuth->bindTo($user->primaryKey)) {
                        throw new Exception(
                          "Error, while binding user to provider:\n\n" . var_export($oAuth->errors, true)
                        );
                    }
    ...
    protected function oAuth($provider)
    {
        try {
            ...
            if ($accessCode === 1) {
                ...
                ?>
              <script>
                  var returnUrl = <?php echo $this->useUserReturnUrl ?
                    CJavaScript::encode(Yii::app()->user->getReturnUrl(false)) : false; ?>;
                  if (window.opener) {
                      if (returnUrl) {
    ...
    protected function prepareGuestUser()
    {
        ...
        if ($this->oAuth->isBond) {
            ...
            Yii::log(
              "Logged in with existing link with '$provider' provider",
              CLogger::LEVEL_INFO,
              'hoauth.' . __CLASS__
            );
            ...
            if ($this->useYiiUser) {
                $user = $this->userModel->findByAttributes(['email' => $userProfile->emailVerified]);
            } else {
     ...
     *    he is logging the first time with this SN. We will ask him for password to validate, that it is realy his
     *    account
    ...
    protected function processUser($user, $userProfile)
    {
        ...
        if ($user->isNewRecord) {
            ...
            if ($this->useYiiUser) {
                ...
                if (!$profile->save()) {
                    throw new Exception(
                      "Error, while saving " . get_class($profile) . " model:\n\n" . var_export($profile->errors, true)
                    );
                }
            }
    ...
    protected function sendActivationEmail($user)
    {
        if ($this->useYiiUser) {
            ...
            if (Yii::app()->getModule('user')->sendActivationMail) {
                ...
                UserModule::sendMail(
                  $user->email,
                  UserModule::t("You registered on {site_name}", ['{site_name}' => Yii::app()->name]),
                  UserModule::t(
                    "To activate your account, please go to {activation_url}",
    ...
    protected function setUp()
    {
        ...
        if (!isset(self::$useYiiUser)) {
            self::$useYiiUser =
              file_exists(Yii::getPathOfAlias('application.modules.user.components') . '/UWrelBelongsTo.php');
        }
    ...
    protected function yiiUserCheckAccess($user, $render = true)
    {
        ...
        if ($error && $render) {
            $this->controller->render('hoauth.views.yiiUserError', [
              'errorCode' => $error,
              'user'      => $user,
            ]);
        }
    ...
    public function getUserModel()
    {
        if ($this->useYiiUser) {
            ...
            $user = call_user_func([$this->model, 'model']);
        }
