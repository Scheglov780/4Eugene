...
class EAssetManager extends CAssetManager
{
    ...
    private function _cacheSet($name, $value)
    {
        ...
        return file_put_contents(
          $this->cachePath . DIRECTORY_SEPARATOR . md5($name) . '.bin',
          serialize($value),
          LOCK_EX
        );
    }
    ...
    public function init()
    {
        if (!Yii::app()->cache) {
            $this->cachePath =
              $this->_getPath(
                $this->cachePath,
                Yii::app()->basePath . DIRECTORY_SEPARATOR . 'runtime' . DIRECTORY_SEPARATOR . 'cache'
              );
        }
        if ($this->lessCompile) {
            if (!$this->lessLib) {
                $this->lessLib =
                  dirname(__FILE__) . DIRECTORY_SEPARATOR . 'EAssetManager' . DIRECTORY_SEPARATOR . 'lessc.inc.php';
            }
            require_once($this->lessLib);
            //Alexy
            if (!is_null($this->lessCompiledPath)) {
    ...
    public function lessCompile($src)
    {
        if (!is_null($this->lessCompiledPath)) {
            ...
            $path = dirname($src) . DIRECTORY_SEPARATOR . basename($src, '.less') . '.css';
        }
        $lessCompile = false;
                ...
                case 'less':
                {
                    ...
                    if (!is_null($this->lessCompiledPath)) {
                        ...
                        /* if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                            $url = str_replace($baseDir . '/', '', $urlPath);
                        } else {
                        */
                        $url = '/' . str_replace($baseDir . '/', '', $urlPath);
                        /* } */
