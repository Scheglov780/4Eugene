...
class PHPExcel_Calculation
{
    /** Constants                */
    /** Regular Expressions        */
    //    Numeric operand    const CALCULATION_REGEXP_CELLREF = CALCULATION_REGEXP_CELLREF;
    //    String operand
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = null)
    {
        ...
        $output = [];
        $expectingOperator =
          false;                    //    We use this test in syntax-checking the expression to determine when a
        //        - is a negation or + is a positive operator rather than an operation
        $expectingOperand =
          false;                    //    We use this test in syntax-checking the expression to determine whether an operand
...
//echo 'Element is a Percentage operator', PHP_EOL;
              ...
              !$expectingOperator) {            //    Positive (unary plus rather than binary operator plus) can be discarded?
//echo 'Element is a Positive number, not Plus operator', PHP_EOL;
                ++$index;                                                    //    Drop the redundant plus symbol
            } elseif ((($opCharacter == '~') || ($opCharacter == '|')) &&
              (!$isOperandOrFunction)) {    //    We have to explicitly deny a tilde or pipe, because they are legal
                return $this->raiseFormulaError(
                  "Formula Error: Illegal character '~'"
                );                //        on the stack but not in the input expression

            } elseif ((isset(self::$operators[$opCharacter]) or $isOperandOrFunction) &&
              $expectingOperator) {    //    Are we putting an operator on the stack?
//echo 'Element with value '.$opCharacter.' is an Operator', PHP_EOL;
                while ($stack->count() > 0 &&
                  ($o2 = $stack->last()) &&
                  isset(self::$operators[$o2['value']]) &&
                  @(self::$operatorAssociativity[$opCharacter] ?
                    self::$operatorPrecedence[$opCharacter] < self::$operatorPrecedence[$o2['value']] :
                    self::$operatorPrecedence[$opCharacter] <= self::$operatorPrecedence[$o2['value']])) {
                    $output[] =
                      $stack->pop(
              ...
              $expectingOperator) {            //    Are we expecting to close a parenthesis?
...
//echo 'Closed Function is '.$functionName, PHP_EOL;
                    $d = $stack->pop();
                    $argumentCount =
                      $d['value'];        //    See how many arguments there were (argument count is the next value stored on the stack)
//if ($argumentCount == 0) {
...
//                    echo 'Element '.$val.' is a Function<br />';
                    if (isset(self::$PHPExcelFunctions[strtoupper($matches[1])]) ||
                      isset(self::$controlFunctions[strtoupper($matches[1])])) {    // it's a function
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if ((is_integer($startRowColRef)) && (ctype_digit($val)) &&
                          ($startRowColRef <= 1048576) && ($val <= 1048576)) {
                            //    Row range
                            $endRowColRef =
                              ($pCellParent !== null) ? $pCellParent->getHighestColumn() :
                          ...
                          (strlen($startRowColRef) <= 3) && (strlen($val) <= 3)) {
                            //    Column range
                            $endRowColRef =
                              ($pCellParent !== null) ? $pCellParent->getHighestRow() :
...
//                        echo 'Element '.$excelConstant.' is an Excel Constant<br />';
                    ...
                    } elseif (($localeConstant = array_search(trim(strtoupper($val)), self::$localeBoolean)) !==
                      false) {
                ...
                switch ($token) {
                    ...
                    case ':':            //    Range
                        ...
                        if ($sheet1 == $sheet2) {
                            ...
                            if ($pCellParent !== null) {
                                $cellValue =
                                  $this->extractCellRange($cellRef, $this->workbook->getSheetByName($sheet1), false);
                            } else {
...
//                            echo '$cellRef='.$cellRef.' in worksheet '.$matches[2].'<br />';
                            ...
                            if ($pCellParent !== null) {
                                $cellValue =
                                  $this->extractCellRange(
                                    $cellRef,
                                    $this->workbook->getSheetByName($matches[2]),
                                    false
                                  );
                            } else {
                                return $this->raiseFormulaError('Unable to access Cell Reference');
                        ...
                        if ($this->_debugLog->getWriteDebugLog()) {
                            ...
                            $this->_debugLog->writeDebugLog(
                              ...
                              implode(
                                ...
                                PHPExcel_Calculation_Functions::flattenArray($argArrayVals)
                              ),
                              ' )'
                            );
                        }
...
//                    echo 'Named Range is '.$namedRange.'<br />';
                    ...
                    $cellValue =
                      $this->extractNamedRange($namedRange, ((null !== $pCell) ? $pCellWorksheet : null), false);
                    $pCell->attach($pCellParent);
                    $this->_debugLog->writeDebugLog(
...
//        }
    ...
    public function calculateCellValue(PHPExcel_Cell $pCell = null, $resetLog = true)
    {
        ...
        try {
            $result =
              self::unwrapResult($this->_calculateFormulaValue($pCell->getValue(), $pCell->getCoordinate(), $pCell));
     ...
     * @return  mixed                Array of values in range if range contains more than one element. Otherwise, a
...
//                echo 'Named Range '.$pRange.' (';
                ...
                if (ctype_alpha($splitRange[0][0])) {
                    $pRange =
                      $splitRange[0][0] . '1:' . $splitRange[0][1] . $namedRange->getWorksheet()->getHighestRow();
