...
class PHPExcel_Calculation_Statistical
{
    ...
    private static function inverseNcdf2($prob)
    {
        ...
        if (abs($y) < 0.42) {
            ...
            $z =
              $c1 + $z * ($c2 + $z * ($c3 + $z * ($c4 + $z * ($c5 + $z * ($c6 + $z * ($c7 + $z * ($c8 + $z * $c9)))))));
            if ($y < 0) {
                $z = -$z;
            }
        }
        ...
        if ((is_numeric($value)) && (is_numeric($trials)) && (is_numeric($probability))) {
            ...
            if ((is_numeric($cumulative)) || (is_bool($cumulative))) {
                if ($cumulative) {
                    $summer = 0;
                    for ($i = 0; $i <= $value; ++$i) {
                        $summer += PHPExcel_Calculation_MathTrig::COMBIN($trials, $i) *
                          pow($probability, $i) *
                          pow(1 - $probability, $trials - $i);
                    }
                    return $summer;
                } else {
                    return PHPExcel_Calculation_MathTrig::COMBIN($trials, $value) *
                      pow($probability, $value) *
                      pow(1 - $probability, $trials - $value);
                }
            }
        }
        return PHPExcel_Calculation_Functions::VALUE();
...
//            $CumPGuessMinus1 = $CumPGuess - $PGuess;
    ...
    public static function HYPGEOMDIST($sampleSuccesses, $sampleNumber, $populationSuccesses, $populationNumber)
    {
        ...
        if ((is_numeric($sampleSuccesses)) &&
          (is_numeric($sampleNumber)) &&
          (is_numeric($populationSuccesses)) &&
          (is_numeric($populationNumber))) {
            if (($sampleSuccesses < 0) ||
              ($sampleSuccesses > $sampleNumber) ||
    ...
    public static function KURT()
    {
        ...
        if ($stdDev > 0) {
            ...
            // Return
            if ($count > 3) {
                return $summer * ($count * ($count + 1) / (($count - 1) * ($count - 2) * ($count - 3))) -
     ...
     * fits your data, and then returns an array that describes the line.
     * @param array of mixed        Data Series Y
     * @param array of mixed        Data Series X
     * @param boolean                A logical value specifying whether to force the intersect to equal 0.
    ...
    public static function NEGBINOMDIST($failures, $successes, $probability)
    {
        ...
        if ((is_numeric($failures)) && (is_numeric($successes)) && (is_numeric($probability))) {
            ...
            return (PHPExcel_Calculation_MathTrig::COMBIN($failures + $successes - 1, $successes - 1)) *
              (pow($probability, $successes)) *
              (pow(1 - $probability, $failures));
        }
        return PHPExcel_Calculation_Functions::VALUE();
    }
    ...
    public static function PERCENTRANK($valueSet, $value, $significance = 3)
    {
        ...
        $value = PHPExcel_Calculation_Functions::flattenSingleValue($value);
        $significance =
          (is_null($significance)) ? 3 : (integer) PHPExcel_Calculation_Functions::flattenSingleValue($significance);
    ...
    public static function PERMUT($numObjs, $numInSet)
    {
        ...
        if ((is_numeric($numObjs)) && (is_numeric($numInSet))) {
            ...
            return round(
              PHPExcel_Calculation_MathTrig::FACT($numObjs) / PHPExcel_Calculation_MathTrig::FACT($numObjs - $numInSet)
            );
        }
        return PHPExcel_Calculation_Functions::VALUE();
    }
        ...
        if ((is_numeric($value)) && (is_numeric($alpha)) && (is_numeric($beta))) {
            ...
            if ((is_numeric($cumulative)) || (is_bool($cumulative))) {
                if ($cumulative) {
                    ...
                    return ($alpha / pow($beta, $alpha)) *
                      pow($value, $alpha - 1) *
                      exp(0 - pow($value / $beta, $alpha));
                }
            }
        }
        return PHPExcel_Calculation_Functions::VALUE();
