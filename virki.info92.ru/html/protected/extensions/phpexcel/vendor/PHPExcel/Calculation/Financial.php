...
class PHPExcel_Calculation_Financial
{
    ...
    public static function COUPNUM($settlement, $maturity, $frequency, $basis = 0)
    {
        ...
        $daysBetweenSettlementAndMaturity =
          PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity, $basis) * 365;
    ...
    public static function DDB($cost, $salvage, $life, $period, $factor = 2.0)
    {
        ...
        if ((is_numeric($cost)) &&
          ...
          (is_numeric($factor))) {
            ...
            for ($per = 1; $per <= $period; ++$per) {
                $depreciation =
                  min(($cost - $previousDepreciation) * ($factor / $life), ($cost - $salvage - $previousDepreciation));
                $previousDepreciation += $depreciation;
            }
            if (PHPExcel_Calculation_Functions::getCompatibilityMode() ==
              PHPExcel_Calculation_Functions::COMPATIBILITY_GNUMERIC) {
     ...
     * @param mixed   $settlement             The security's settlement date.
     *                                        The security settlement date is the date after the issue date when the
     ...
     * constant interest rate. Excel Function: IPMT(rate,per,nper,pv[,fv][,type])
    ...
    public static function NPER($rate = 0, $pmt = 0, $pv = 0, $fv = 0, $type = 0)
    {
        ...
        if (!is_null($rate) && $rate != 0) {
            ...
            return log(($pmt * (1 + $rate * $type) / $rate - $fv) / ($pv + $pmt * (1 + $rate * $type) / $rate)) /
              log(1 + $rate);
        }
        if ($pmt == 0) {
            return PHPExcel_Calculation_Functions::NaN();
    ...
    public static function TBILLEQ($settlement, $maturity, $discount)
    {
        ...
        if (PHPExcel_Calculation_Functions::getCompatibilityMode() ==
          PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) {
            ...
            $daysBetweenSettlementAndMaturity =
              (PHPExcel_Calculation_DateTime::getDateValue($maturity) -
                PHPExcel_Calculation_DateTime::getDateValue($settlement));
        }
        ...
        if (is_numeric($discount)) {
            ...
            if (PHPExcel_Calculation_Functions::getCompatibilityMode() ==
              PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) {
                ++$maturity;
                $daysBetweenSettlementAndMaturity =
                  PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity) * 360;
                if (!is_numeric($daysBetweenSettlementAndMaturity)) {
                    //    return date error
                    return $daysBetweenSettlementAndMaturity;
                }
     ...
     *                                         The first payment is optional and corresponds to a cost or payment that
     *                                         occurs at the beginning of the investment. If the first value is a cost
     *                                         or payment, it must be a negative value. All succeeding payments are
     *                                         discounted based on a 365-day year. The series of values must contain at
    ...
    public static function XNPV($rate, $values, $dates)
    {
        ...
        for ($i = 0; $i < $valCount; ++$i) {
            ...
            $xnpv += $values[$i] /
              pow(1 + $rate, PHPExcel_Calculation_DateTime::DATEDIF($dates[0], $dates[$i], 'd') / 365);
        }
    ...
    public static function YIELDMAT($settlement, $maturity, $issue, $rate, $price, $basis = 0)
    {
        ...
        if (is_numeric($rate) && is_numeric($price)) {
            ...
            return ((1 +
                  (($daysBetweenIssueAndMaturity / $daysPerYear) * $rate) -
