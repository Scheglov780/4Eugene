...
class PclZip
{
    ...
    public function add($p_filelist)
    {
        ...
        foreach ($v_att_list as $v_entry) {
            $v_result =
              $this->privFileDescrParseAtt($v_entry, $v_filedescr_list[], $v_options, $v_supported_attributes);
            if ($v_result != 1) {
                return 0;
            }
        }
    ...
    public function extractByIndex($p_index)
    {
        ...
        // ----- Call the extracting fct
        if (($v_result = $this->privExtractByRule($p_list, $v_path, $v_remove_path, $v_remove_all_path, $v_options)) <
          1) {
            return (0);
        }
    ...
    public function privAdd($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        if (($v_result =
            $this->privWriteCentralHeader($v_count + $v_central_dir['entries'], $v_size, $v_offset, $v_comment)) != 1) {
            // ----- Reset the file list
            unset($v_header_list);
            $this->privSwapBackMagicQuotes();
        ...
        for ($j = 0; ($j < sizeof($p_filedescr_list)) && ($v_result == 1); $j++) {
            ...
            if (($p_filedescr_list[$j]['type'] == 'file') ||
              ...
              (($p_filedescr_list[$j]['type'] == 'folder') &&
                (!isset($p_options[PCLZIP_OPT_REMOVE_ALL_PATH]) || !$p_options[PCLZIP_OPT_REMOVE_ALL_PATH]))) {
                // ----- Add the file
    ...
    public function privDeleteByRule(&$p_result_list, &$p_options)
    {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($j = 0; ($j < sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $j++) {
                    ...
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                        ...
                        if ((strlen($v_header_list[$v_nb_extracted]['stored_filename']) >
        ...
        if ($v_nb_extracted > 0) {
            ...
            for ($i = 0; $i < sizeof($v_header_list); $i++) {
                ...
                if (($v_result =
                    PclZipUtilCopyBlock($this->zip_fd, $v_temp_zip->zip_fd, $v_header_list[$i]['compressed_size'])) !=
                  1) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                    $v_temp_zip->privCloseFd();
            ...
            // ----- Create the central dir footer
            if (($v_result =
                $v_temp_zip->privWriteCentralHeader(sizeof($v_header_list), $v_size, $v_offset, $v_comment)) != 1) {
                // ----- Reset the file list
                unset($v_header_list);
                $v_temp_zip->privCloseFd();
                $this->privCloseFd();
    ...
    public function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        // ----- Check the path
        if (($p_path == "") ||
          ((substr($p_path, 0, 1) != "/") && (substr($p_path, 0, 3) != "../") && (substr($p_path, 1, 2) != ":/"))) {
            $p_path = "./" . $p_path;
        }
            ...
            if (($v_extract) && (($v_header['compression'] != 8) && ($v_header['compression'] != 0))) {
                ...
                if ((isset($p_options[PCLZIP_OPT_STOP_ON_ERROR])) && ($p_options[PCLZIP_OPT_STOP_ON_ERROR] === true)) {
                    ...
                    PclZip::privErrorLog(
                      ...
                      "' is compressed by an unsupported compression method (" .
            ...
            if ($v_extract) {
                ...
                if ($p_options[PCLZIP_OPT_EXTRACT_AS_STRING]) {
                  ...
                  ($p_options[PCLZIP_OPT_EXTRACT_IN_OUTPUT])) {
                    ...
                    // ----- Extracting the file
                    $v_result1 =
                      $this->privExtractFile($v_header, $p_path, $p_remove_path, $p_remove_all_path, $p_options);
                    if ($v_result1 < 1) {
                        $this->privCloseFd();
                        $this->privSwapBackMagicQuotes();
                        return $v_result1;
    ...
    public function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if (($v_result =
                    $this->privDirCheck($v_dir_to_check, (($p_entry['external'] & 0x00000010) == 0x00000010))) != 1) {
                    // ----- Change the file status
    ...
    public function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
    {
        ...
        // ----- Packed data
        $v_binary_data =
          pack("VvvvvVVv", 0x06054b50, 0, 0, $p_nb_entries, $p_nb_entries, $p_size, $p_offset, strlen($p_comment));
...
function PclZipUtilOptionText($p_option)
{
    ...
    for (reset($v_list); $v_key = key($v_list); next($v_list)) {
        ...
        if ((($v_prefix == 'PCLZIP_OPT') || ($v_prefix == 'PCLZIP_CB_') || ($v_prefix == 'PCLZIP_ATT')) &&
