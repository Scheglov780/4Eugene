...
class PHPExcel_Chart_Renderer_jpgraph
{
    ...
    private function formatDataSetLabels($groupID, $datasetLabels, $labelCount, $rotation = '')
    {
        ...
        foreach ($datasetLabels as $i => $datasetLabel) {
            if (is_array($datasetLabel)) {
                ...
                if (!is_null($datasetLabelFormatCode)) {
                    $datasetLabels[$i] =
                      PHPExcel_Style_NumberFormat::toFormattedString($datasetLabel, $datasetLabelFormatCode);
                }
            }
            ++$testCurrentIndex;
        }
    ...
    private function renderPlotBar($groupID, $dimensions = '2d')
    {
        ...
        $labelCount =
          count($this->chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotValuesByIndex(0)->getPointCount());
        if ($labelCount > 0) {
            $datasetLabels =
