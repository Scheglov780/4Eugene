...
class PHPExcel_Reader_SYLK extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            $rowData =
              explode("\t", str_replace('¤', ';', str_replace(';', "\t", str_replace(';;', '¤', rtrim($rowData)))));
        ...
        $worksheetInfo[0]['lastColumnLetter'] =
          PHPExcel_Cell::stringFromColumnIndex($worksheetInfo[0]['lastColumnIndex']);
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            if ($dataType == 'P') {
                            ...
                            foreach ($temp as &$value) {
                                ...
                                if ($key = !$key) {
                                    ...
                                    foreach ($cellReferences as $cellReference) {
                                        ...
                                        $A1CellReference =
                                          PHPExcel_Cell::stringFromColumnIndex($columnReference - 1) . $rowReference;
                ...
                // Set cell value
                $objPHPExcel->getActiveSheet()->getCell($columnLetter . $row)->setValue(
                  ($hasCalculatedValue) ? $cellDataFormula : $cellData
                );
                ...
                if (($formatStyle > '') && ($column > '') && ($row > '')) {
                    ...
                    if (isset($this->formats[$formatStyle])) {
                        $objPHPExcel->getActiveSheet()->getStyle($columnLetter . $row)->applyFromArray(
                          $this->formats[$formatStyle]
                        );
                    }
                }
