        ...
        foreach ($tokens as $token) {
            ...
            switch ($token['name']) {
                ...
                case 'tArray': // array constant
                    ...
                    $additionalData =
                      substr($additionalData, $constantArray['size']); // bite of chunk of additional data
                    unset($space0, $space1);
                    break;
                case 'tMemArea':
...
//        $this->userDefinedProperties = $ole->getUserDefinedProperties();
    ...
    private function readBlank()
    {
        ...
        // Read cell?
        if (($this->getReadFilter() !== null) &&
          $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
                ...
                case 0x40:    //    Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    ...
                    $value =
                      PHPExcel_Shared_OLE::OLE2LocalDate(
                        substr($this->documentSummaryInformation, $secOffset + 4 + $offset, 8)
                      );
                    break;
                    ...
                    if (isset($fmtRuns[$i])) {
                        ...
                        $text =
                          PHPExcel_Shared_String::Substring(
                            $this->sst[$index]['value'],
                            $charPos,
                            PHPExcel_Shared_String::CountCharacters($this->sst[$index]['value'])
                          );
                    }
    ...
    private function readPageSetup()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (!$isNotInit) {
                ...
                switch ($isPortrait) {
                    case 0:
                        $this->phpSheet->getPageSetup()->setOrientation(
        ...
        if (isset($this->ref[$index])) {
            ...
            switch ($type) {
                case 'internal':
                    ...
                    if ($this->ref[$index]['firstSheetIndex'] == 0xFFFF or
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    ...
                    $value =
                      PHPExcel_Shared_OLE::OLE2LocalDate(
                        substr($this->summaryInformation, $secOffset + 4 + $offset, 8)
                      );
                    break;
                case 0x47: // Clipboard format
                    // not needed yet, fix later if necessary
...
//        echo '<br />';
    ...
    private function readWindow2()
    {
        ...
        if ($this->phpSheet->getSheetView()->getView() !== PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
            ...
            $view =
              $isPageBreakPreview ? PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW :
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $objStyle->getProtection()->setLocked(
              $isLocked ? PHPExcel_Style_Protection::PROTECTION_INHERIT :
            ...
            $objStyle->getProtection()->setHidden(
              $isHidden ? PHPExcel_Style_Protection::PROTECTION_PROTECTED :
                PHPExcel_Style_Protection::PROTECTION_UNPROTECTED
            );
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if ($bordersLeftStyle =
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x0000000F & self::getInt4d($recordData, 10)) >> 0)) {
                    $objStyle->getBorders()->getLeft()->setBorderStyle($bordersLeftStyle);
                }
                // bit: 7-4; mask: 0x000000F0; right style
                if ($bordersRightStyle =
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x000000F0 & self::getInt4d($recordData, 10)) >> 4)) {
                    $objStyle->getBorders()->getRight()->setBorderStyle($bordersRightStyle);
                }
                // bit: 11-8; mask: 0x00000F00; top style
                if ($bordersTopStyle =
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x00000F00 & self::getInt4d($recordData, 10)) >> 8)) {
                    $objStyle->getBorders()->getTop()->setBorderStyle($bordersTopStyle);
                }
                // bit: 15-12; mask: 0x0000F000; bottom style
                if ($bordersBottomStyle =
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x0000F000 & self::getInt4d($recordData, 10)) >> 12)) {
                ...
                if ($bordersDiagonalStyle =
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x01E00000 & self::getInt4d($recordData, 14)) >> 21)) {
                    $objStyle->getBorders()->getDiagonal()->setBorderStyle($bordersDiagonalStyle);
                }

                // bit: 31-26; mask: 0xFC000000 fill pattern
                if ($fillType =
                  PHPExcel_Reader_Excel5_Style_FillPattern::lookup(
                    (0xFC000000 & self::getInt4d($recordData, 14)) >> 26
                  )) {
                ...
                $objStyle->getFill()->setFillType(
                  PHPExcel_Reader_Excel5_Style_FillPattern::lookup((0x003F0000 & $borderAndBackground) >> 16)
                );

                // bit: 24-22; mask: 0x01C00000; bottom line style
                $objStyle->getBorders()->getBottom()->setBorderStyle(
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x01C00000 & $borderAndBackground) >> 22)
                );
                ...
                $objStyle->getBorders()->getTop()->setBorderStyle(
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x00000007 & $borderLines) >> 0)
                );

                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()->getLeft()->setBorderStyle(
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x00000038 & $borderLines) >> 3)
                );

                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()->getRight()->setBorderStyle(
                  PHPExcel_Reader_Excel5_Style_Border::lookup((0x000001C0 & $borderLines) >> 6)
                );
                ...
                switch ($extType) {
                    ...
                    case 7:        // border color top
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $top =
                                  $this->phpExcel->getCellXfByIndex($this->mapCellXfIndex[$ixfe])->getBorders()->getTop(
                                  );
                    ...
                    case 8:        // border color bottom
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                $bottom =
                                  $this->phpExcel->getCellXfByIndex($this->mapCellXfIndex[$ixfe])
                                    ->getBorders()
                                    ->getBottom();
        ...
        if (!$this->readDataOnly) {
            ...
            foreach ($this->phpExcel->getCellXfCollection() as $objStyle) {
                ...
                if (isset($fill->startcolorIndex)) {
                    $startColor =
                      PHPExcel_Reader_Excel5_Color::map($fill->startcolorIndex, $this->palette, $this->version);
                ...
                if (isset($top->colorIndex)) {
                    $borderTopColor =
                      PHPExcel_Reader_Excel5_Color::map($top->colorIndex, $this->palette, $this->version);
                    $top->getColor()->setRGB($borderTopColor['rgb']);
                }
                if (isset($right->colorIndex)) {
                    $borderRightColor =
                      PHPExcel_Reader_Excel5_Color::map($right->colorIndex, $this->palette, $this->version);
                    $right->getColor()->setRGB($borderRightColor['rgb']);
                }
                if (isset($bottom->colorIndex)) {
                    $borderBottomColor =
                      PHPExcel_Reader_Excel5_Color::map($bottom->colorIndex, $this->palette, $this->version);
                    $bottom->getColor()->setRGB($borderBottomColor['rgb']);
                }
                if (isset($left->colorIndex)) {
                    $borderLeftColor =
                      PHPExcel_Reader_Excel5_Color::map($left->colorIndex, $this->palette, $this->version);
                    $left->getColor()->setRGB($borderLeftColor['rgb']);
                }
                if (isset($diagonal->colorIndex)) {
                    $borderDiagonalColor =
                      PHPExcel_Reader_Excel5_Color::map($diagonal->colorIndex, $this->palette, $this->version);
...
//                echo '<br />';
                ...
                if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    [$startColumn, $startRow] =
                      PHPExcel_Cell::coordinateFromString($spContainer->getStartCoordinates());
...
//                            echo 'Picture Object<br />';
            ...
            if ($this->version == self::XLS_BIFF8) {
                foreach ($this->sharedFormulaParts as $cell => $baseCell) {
                    ...
                    if (($this->getReadFilter() !== null) &&
                      $this->getReadFilter()->readCell($column, $row, $this->phpSheet->getTitle())) {
...
//                    echo '<br />';
                    ...
                    $this->phpSheet->getComment($cellAddress)->setAuthor($noteDetails['author'])->setText(
                      $this->parseRichText($noteDetails['objTextData']['text'])
                    );
                }
            }
        }
                                ...
                                if ($docSheet = $this->phpExcel->getSheetByName($explodes[0])) {
                                    ...
                                    if (count($coordinateStrings) == 2) {
                                        [$firstColumn, $firstRow] =
                                          PHPExcel_Cell::coordinateFromString($coordinateStrings[0]);
                                        [$lastColumn, $lastRow] =
                                          PHPExcel_Cell::coordinateFromString($coordinateStrings[1]);
                ...
                if (count($explodes) == 2) {
                    if (($docSheet = $this->phpExcel->getSheetByName($explodes[0])) ||
                      ($docSheet = $this->phpExcel->getSheetByName(trim($explodes[0], "'")))) {
                        ...
                        $scope =
                          ($definedName['scope'] == 0) ? null :
                            $this->phpExcel->getSheetByName($this->sheets[$definedName['scope'] - 1]['name']);
    ...
    private static function readUnicodeString($subData, $characterCount)
    {
        ...
        // needs to be fixed
        $value =
          self::encodeUTF16(substr($subData, 1, $isCompressed ? $characterCount : 2 * $characterCount), $isCompressed);
