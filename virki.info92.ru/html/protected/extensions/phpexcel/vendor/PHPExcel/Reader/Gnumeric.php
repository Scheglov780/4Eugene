...
class PHPExcel_Reader_Gnumeric extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        $xml = new XMLReader();
        $xml->xml(
          $this->securityScanFile('compress.zlib://' . realpath($pFilename)),
          null,
            ...
            if ((!$this->readDataOnly) && (isset($sheet->Objects))) {
                foreach ($sheet->Objects->children('gnm', true) as $key => $comment) {
                    ...
                    if ($commentAttributes->Text) {
                        $objPHPExcel->getActiveSheet()->getComment((string) $commentAttributes->ObjectBound)->setAuthor(
                      ...
                      (PHPExcel_Shared_Date::isDateTimeFormatCode((string) $styleAttributes['Format']))) {
                        ...
                        if (!$this->readDataOnly) {
                            ...
                            $styleArray['alignment']['shrinkToFit'] =
                              ($styleAttributes['ShrinkToFit'] == '1') ? true : false;
                            $styleArray['alignment']['indent'] =
                              (intval($styleAttributes["Indent"]) > 0) ? $styleAttributes["indent"] : 0;
                            ...
                            if (isset($styleRegion->Style->StyleBorder)) {
                                ...
                                if (isset($styleRegion->Style->StyleBorder->Bottom)) {
                                    $styleArray['borders']['bottom'] =
                                      self::parseBorderAttributes(
                                        $styleRegion->Style->StyleBorder->Bottom->attributes()
                                      );
                                }
                                ...
                                if (isset($styleRegion->Style->StyleBorder->Right)) {
                                    $styleArray['borders']['right'] =
                                      self::parseBorderAttributes(
                                        $styleRegion->Style->StyleBorder->Right->attributes()
                                      );
                                }
                                if ((isset($styleRegion->Style->StyleBorder->Diagonal)) &&
                                  (isset($styleRegion->Style->StyleBorder->{'Rev-Diagonal'}))) {
                                    $styleArray['borders']['diagonal'] =
                                      self::parseBorderAttributes(
                                        $styleRegion->Style->StyleBorder->Diagonal->attributes()
                                      );
                                    ...
                                    $styleArray['borders']['diagonal'] =
                                      self::parseBorderAttributes(
                                        $styleRegion->Style->StyleBorder->{'Rev-Diagonal'}->attributes()
                                      );
...
//                        echo '<br />';
            ...
            if ((!$this->readDataOnly) && (isset($sheet->Cols))) {
                ...
                foreach ($sheet->Cols->ColInfo as $columnOverride) {
                    ...
                    $hidden =
                      ((isset($columnAttributes['Hidden'])) && ($columnAttributes['Hidden'] == '1')) ? true : false;
