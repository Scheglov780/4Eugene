                    ...
                    do {
                        ...
                        if ($xml->name == 'table:table-row' && $xml->nodeType == XMLReader::ELEMENT) {
                            ...
                            do {
                                ...
                                } elseif ($xml->name == 'table:covered-table-cell' &&
                        ...
                        foreach ($propertyValueAttributes as $key => $value) {
                            if ($key == 'name') {
                                ...
                                switch ($value) {
                                    case 'date':
                                        $propertyValue =
                                          PHPExcel_DocumentProperties::convertProperty($propertyValue, 'date');
                                        $propertyValueType = PHPExcel_DocumentProperties::PROPERTY_TYPE_DATE;
                                        break;
                                    case 'boolean':
                                        $propertyValue =
                                          PHPExcel_DocumentProperties::convertProperty($propertyValue, 'bool');
                                        $propertyValueType = PHPExcel_DocumentProperties::PROPERTY_TYPE_BOOLEAN;
                                        break;
                                    case 'float':
                                        $propertyValue =
                                          PHPExcel_DocumentProperties::convertProperty($propertyValue, 'r4');
...
//                    echo '<b>'.$key.'</b><br />';
                    switch ($key) {
                        ...
                        case 'table-row':
                            $rowDataTableAttributes = $rowData->attributes($namespacesContent['table']);
                            $rowRepeats =
                              (isset($rowDataTableAttributes['number-rows-repeated'])) ?
                                $rowDataTableAttributes['number-rows-repeated'] : 1;
                            $columnID = 'A';
                            foreach ($rowData as $key => $cellData) {
...
//                                    echo 'Cell has comment<br />';
                                    ...
                                    $text = implode("\n", $textArray);
//                                    echo $text, '<br />';
                                    $objPHPExcel->getActiveSheet()->getComment($columnID . $rowID)->setText(
                                    ...
                                    switch ($cellDataOfficeAttributes['value-type']) {
                                        case 'string':
                                            ...
                                            if (isset($dataValue->a)) {
                                                ...
                                                $cellXLinkAttributes =
                                                  $dataValue->attributes($namespacesContent['xlink']);
                                        ...
                                        case 'date':
                                            ...
                                            [$year, $month, $day, $hour, $minute, $second] =
                                              explode(' ', $dateObj->format('Y m d H i s'));
                                            $dataValue =
                                            ...
                                            $dataValue =
                                              PHPExcel_Shared_Date::PHPToExcel(
                                                strtotime(
                                                  ...
                                                  implode(
                                                    ':',
                                                    sscanf($cellDataOfficeAttributes['time-value'], 'PT%dH%dM%dS')
                                                  )
                                                )
                                              );
...
//                                    echo 'Formula: ', $cellDataFormula, PHP_EOL;
                                    ...
                                    foreach ($temp as &$value) {
                                        ...
                                        if ($tKey = !$tKey) {
                                            ...
                                            $value =
                                              PHPExcel_Calculation::translateSeparator(';', ',', $value, $inBraces);
                                        }
                                    }
                                    unset($value);
...
//                                    echo 'Adjusted Formula: ', $cellDataFormula, PHP_EOL;
                                ...
                                $colRepeats =
                                  (isset($cellDataTableAttributes['number-columns-repeated'])) ?
                                    $cellDataTableAttributes['number-columns-repeated'] : 1;
                                if ($type !== null) {
                                        ...
                                        if ($type !== PHPExcel_Cell_DataType::TYPE_NULL) {
                                            for ($rowAdjust = 0; $rowAdjust < $rowRepeats; ++$rowAdjust) {
                                                ...
                                                $objPHPExcel->getActiveSheet()
                                                  ...
                                                  ->setValueExplicit(
                                                    (($hasCalculatedValue) ? $cellDataFormula : $dataValue),
                                                    $type
                                                  );
                                                if ($hasCalculatedValue) {
//                                                    echo 'Forumla result is '.$dataValue.'<br />';
                                ...
                                if ((isset($cellDataTableAttributes['number-columns-spanned'])) ||
                                  (isset($cellDataTableAttributes['number-rows-spanned']))) {
                                    if (($type !== PHPExcel_Cell_DataType::TYPE_NULL) || (!$this->readDataOnly)) {
                                        $columnTo = $columnID;
                                        if (isset($cellDataTableAttributes['number-columns-spanned'])) {
                                            $columnTo =
                                              PHPExcel_Cell::stringFromColumnIndex(
                                                PHPExcel_Cell::columnIndexFromString($columnID) +
                                                $cellDataTableAttributes['number-columns-spanned'] -
                                                2
                                              );
                                        }
                                        $rowTo = $rowID;
