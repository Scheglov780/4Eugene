...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    private function createNewCell($pCoordinate)
    {
        ...
        if (PHPExcel_Cell::columnIndexFromString($this->cachedHighestColumn) <
          PHPExcel_Cell::columnIndexFromString($aCoordinates[0])) {
    ...
    public function cellExists($pCoordinate = 'A1')
    {
        ...
        if (strpos($pCoordinate, '!') !== false) {
            ...
            return $this->parent->getSheetByName($worksheetReference[0])->cellExists(
              strtoupper($worksheetReference[1])
            );
        }
...
//
    ...
    public function getCell($pCoordinate = 'A1', $createIfNotExists = true)
    {
        ...
        if (strpos($pCoordinate, '!') !== false) {
            $worksheetReference = PHPExcel_Worksheet::extractSheetTitle($pCoordinate, true);
            return $this->parent->getSheetByName($worksheetReference[0])->getCell(
              strtoupper($worksheetReference[1]),
              $createIfNotExists
            );
        }
     ...
     * @param null|string Same rule as Title minus space not allowed (but, like Excel, change silently space to
     *                         underscore)
     * @return objWorksheet
     * @throws PHPExcel_Exception
     */
    public function setCodeName($pValue = null)
    {
        ...
        $pValue =
          str_replace(' ', '_', $pValue);//Excel does this automatically without flinching, we are doing the same
        // Syntax check
    ...
    public function getColumnDimension($pColumn = 'A', $create = true)
    {
        ...
        if (!isset($this->columnDimensions[$pColumn])) {
            ...
            if (PHPExcel_Cell::columnIndexFromString($this->cachedHighestColumn) <
              PHPExcel_Cell::columnIndexFromString($pColumn)) {
    ...
    public function getHashCode()
    {
        if ($this->dirty) {
            $this->hash =
              md5(
                $this->title . $this->autoFilter . ($this->protection->isProtectionEnabled() ? 't' : 'f') . __CLASS__
              );
            $this->dirty = false;
        }
        return $this->hash;
    ...
    public function getStyleByColumnAndRow($pColumn = 0, $pRow = 1, $pColumn2 = null, $pRow2 = null)
    {
        if (!is_null($pColumn2) && !is_null($pRow2)) {
            $cellRange =
              PHPExcel_Cell::stringFromColumnIndex($pColumn) .
              $pRow .
              ':' .
              PHPExcel_Cell::stringFromColumnIndex($pColumn2) .
              $pRow2;
    ...
    {
        $cellRange =
          PHPExcel_Cell::stringFromColumnIndex($pColumn1) .
          $pRow1 .
          ':' .
          PHPExcel_Cell::stringFromColumnIndex($pColumn2) .
          $pRow2;
     ...
     * @param boolean $returnCellRef     False - Return a simple array of rows and columns indexed by number counting
     *                                   from zero True - Return rows and columns indexed by their actual row and
     *                                   column IDs
     * @return array
     * @throws PHPExcel_Exception
     */
     ...
     * @throws    PHPExcel_Exception
     */
    public function protectCellsByColumnAndRow(
      $pColumn1 = 0,
      $pRow1 = 1,
      $pColumn2 = 0,
      $pRow2 = 1,
                    ...
                    if ($cell->getValue() !== null) {
                        ...
                        if ($formatData) {
                            ...
                            $returnValue[$rRef][$cRef] = PHPExcel_Style_NumberFormat::toFormattedString(
                              ...
                              ($style && $style->getNumberFormat()) ? $style->getNumberFormat()->getFormatCode() :
    ...
    public function removeColumn($pColumn = 'A', $pNumCols = 1)
    {
        if (!is_numeric($pColumn)) {
            ...
            $pColumn =
              PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($pColumn) - 1 + $pNumCols);
            $objReferenceHelper = PHPExcel_ReferenceHelper::getInstance();
            $objReferenceHelper->insertNewBefore($pColumn . '1', -$pNumCols, 0, $this);
            for ($c = 0; $c < $pNumCols; ++$c) {
                $this->getCellCacheController()->removeColumn($highestColumn);
                $highestColumn =
                  PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($highestColumn) - 2);
            }
        } else {
            throw new PHPExcel_Exception("Column references should not be numeric.");
        }
    ...
    public function shrinkRangeToFit($range)
    {
        ...
        foreach ($rangeBlocks as &$rangeSet) {
            ...
            $rangeSet =
              $rangeBoundaries[0][0] . $rangeBoundaries[0][1] . ':' . $rangeBoundaries[1][0] . $rangeBoundaries[1][1];
        }
        unset($rangeSet);
     ...
     * @throws    PHPExcel_Exception
     */
    public function unprotectCellsByColumnAndRow(
      $pColumn1 = 0,
      $pRow1 = 1,
      $pColumn2 = 0,
      $pRow2 = 1,
