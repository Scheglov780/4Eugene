...
class Booster extends CApplicationComponent
{
    ...
    protected function addOurPackagesToYii()
    {
        ...
        $this->cs->scriptMap['jquery-ui.js'] = $this->getAssetsUrl() . '/js/jquery-ui-no-conflict.js';
        $this->cs->scriptMap['jquery-ui.min.js'] = $this->getAssetsUrl() . '/js/jquery-ui-no-conflict.min.js';
        $this->cs->scriptMap['jquery.yiigridview.js'] =
          $this->getAssetsUrl() . (YII_DEBUG ? '/js/jquery.dsyiigridview.js' : '/js/jquery.dsyiigridview.min.js');
        $this->cs->scriptMap['jquery.yiilistview.js'] =
          $this->getAssetsUrl() . (YII_DEBUG ? '/js/jquery.dsyiilistview.js' : '/js/jquery.dsyiilistview.min.js');
    }
    ...
    protected function createBootstrapCssPackage()
    {
        ...
        return [
          'bootstrap.css' => [
            'baseUrl' => $this->enableCdn ? '//netdna.bootstrapcdn.com/bootstrap/3.4.1/' :
              $this->getAssetsUrl() . '/bootstrap/',
            'css'     => [($this->minify || $this->enableCdn) ? 'css/bootstrap.min.css' : 'css/bootstrap.css'],
          ],
        ];
    }
        ...
        if (isset($this->_assetsUrl)) {
            ...
            return $this->_assetsUrl =
              Yii::app()->getAssetManager()->publish(
                Yii::getPathOfAlias('booster.assets'),
                false,
                -1,
    ...
    public function registerJQueryCss()
    {
        ...
        $this->registerPackage('jquery-css')->scriptMap['jquery-ui.css'] =
          $this->getAssetsUrl() . '/css/jquery-ui-bootstrap.css';
    }

    /**
     * Registers a script package that is listed in {@link packages}.
     * @param string $name   the name of the script package.
     * @return CClientScript the CClientScript object itself (to support method chaining, available since version
     *                       1.1.5).
