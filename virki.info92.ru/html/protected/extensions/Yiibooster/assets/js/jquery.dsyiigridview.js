	...
	methods = {
		...
		init: function (options) {
			...
			return this.each(function () {
				...
				var inputSelector = '#' + id + ' .tableFloatingHeaderOriginal .' + settings.filterClass + ' input, ' + '#' + id + ' .tableFloatingHeaderOriginal .' + settings.filterClass + ' select';
				...
				// Alexy on-off fix for jquery events
				...
				// Alexy on-off fix for jquery events
				$(document).off('change.yiiGridView keydown.yiiGridView',settings.filterSelector).on('change.yiiGridView keydown.yiiGridView', settings.filterSelector, function (event) {
				...
				if (settings.selectableRows > 0) {
					...
					$(document).off('click.yiiGridView', '#' + id + ' .' + settings.tableClass + ' > tbody > tr').on('click.yiiGridView', '#' + id + ' .' + settings.tableClass + ' > tbody > tr', function (e) {
						...
						if (settings.selectableRows === 1) {
							...
							// Alexy was: $checks.prop('checked', false);
							$checks.prop('checked', false).trigger("change");
						}
						// Alexy was: $('input.select-on-check', $row).prop('checked', isRowSelected);
