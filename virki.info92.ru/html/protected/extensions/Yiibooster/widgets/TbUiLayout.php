...
class TbUiLayout extends CWidget
{
    ...
    {
        $layoutsOptions = [];
        $availableLayouts = ['center', 'north', 'south', 'east', 'west'];
        ...
        foreach ($availableLayouts as $layoutName) {
            ...
            $htmlOptions = !empty($layout['htmlOptions']) ? $layout['htmlOptions'] : [];
    ...
    public function run()
    {
        ...
        $cs->registerScript(
          __CLASS__ . '#jsVar#' . $this->getId(),
          'var ' . $this->jsVarName . ';',
          CClientScript::POS_HEAD
        );
        // Register Layouts init script
        $cs->registerScript(
          __CLASS__ . '#init#' . $this->getId(),
          $this->jsVarName . ' = $("#' . $id . '").layout(' . $options . ');',
