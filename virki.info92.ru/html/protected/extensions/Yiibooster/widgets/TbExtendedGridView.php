...
class TbExtendedGridView extends TbGridView
{
    ...
    /**
     * @var string the filter to use to allow selection. For example, if you set the "htmlOptions" property of a column
     *      to have a
    ...
    protected function getPrimaryKey($data)
    {
        if ($this->dataProvider instanceof CActiveDataProvider) {
            $key =
              $this->dataProvider->keyAttribute === null ? $data->getPrimaryKey() :
                $data->{$this->dataProvider->keyAttribute};
            return is_array($key) ? implode(',', $key) : $key;
        }
        if (($this->dataProvider instanceof CArrayDataProvider || $this->dataProvider instanceof CSqlDataProvider) &&
          !empty($this->dataProvider->keyField)) {
    ...
    public function init()
    {
        ...
        if ($this->bulkActions !== [] && isset($this->bulkActions['actionButtons'])) {
        ...
        $cs->registerScript(
          ...
          '
			$.ajaxPrefilter(function (options, originalOptions, jqXHR) {
				...
				if (qs.hasOwnProperty("ajax") && qs.ajax == "' .
          $this->id .
          '")
				{
    ...
    public function renderChart()
    {
          ...
          [
            ...
            'buttons'     => [
              [
                ...
                'url'         => '#',
                'htmlOptions' => ['class' => 'active ' . $this->getId() . '-grid-control grid'],
              ],
              [
                'label'       => Yii::t('zii', 'Chart'),
                'url'         => '#',
                'htmlOptions' => ['class' => $this->getId() . '-grid-control chart'],
              ],
            ],
            'htmlOptions' => ['style' => 'margin-bottom:5px'],
          ]
        );
        echo '<div>';
        $buttons->init();
        ...
        foreach ($configSeries as $set) {
            $seriesData[$cnt] = ['name' => isset($set['name']) ? $set['name'] : null, 'data' => []];
        ...
        $options = CMap::mergeArray(
          $this->chartOptions['config'],
          ['series' => $seriesData, 'xAxis' => $xAxisData]
        );
        ...
        if (empty($this->chartOptions['htmlOptions']['style'])) {
            ...
            $this->chartOptions['htmlOptions']['style'] =
              $this->chartOptions['htmlOptions']['style'] . '; width: 100%; height: 100%;';
        }
    ...
    public function renderKeys()
    {
        ...
        if (!$this->sortableRows ||
          (isset($data[0]) && !isset($data[0]->attributes[(string) $this->sortableAttribute]))) {
            parent::renderKeys();
        }
        ...
        foreach ($data as $d) {
            echo CHtml::tag(
              'span',
              ['data-order' => $this->getAttribute($d, $this->sortableAttribute)],
 ...
 * Renders a summary based on the percent count of specified types. For example, if a value has a type 'blue', this
 * class will count the percentage number of times the value 'blue' has on that column.
 * @package booster.widgets.grids.operations
 */
