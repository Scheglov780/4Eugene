...
class TbToggleColumn extends TbDataColumn
{
     ...
     * <ul>
     * <li><code>success</code> status of the ajax call, true if the ajax call was successful, false if the ajax call
     * failed.
     * <li><code>data</code> the data returned by the server in case of a successful call or XHR object in case of
     * error.
     * </ul>
    ...
    /**
     * @var string the attribute name of the data model. Used for column sorting, filtering and to render the
     *      corresponding attribute value in each data cell. If {@link value} is specified it will be used to rendered
     *      the data cell instead of the attribute value.
     * @see value
     * @see sortable
     */
    public $name;
    /**
     * @var boolean whether the column is sortable. If so, the header cell will contain a link that may trigger the
     *      sorting. Defaults to true. Note that if {@link name} is not set, or if {@link name} is not allowed by
     *      {@link CSort}, this property will be treated as false.
     * @see name
     */
    ...
    protected function initButton()
    {
        ...
        $this->button = [
          'url'         => 'Yii::app()->controller->createUrl("' .
            $this->toggleAction .
            '",array("pk"=>$data->primaryKey,"attribute"=>"' .
            $this->name .
            '"))',
...
function() {
    ...
    protected function renderDataCellContent($row, $data)
    {
        ...
        $checked = ($this->value === null)
          ...
          : $this->evaluateExpression($this->value, ['data' => $data, 'row' => $row]);
