...
class elFinderVolumeFTP extends elFinderVolumeDriver
{
    ...
    protected function _stat($path)
    {
        ...
        if (is_array($raw) && count($raw) > 1 && substr(trim($raw[0]), 0, 1) == 2) {
            ...
            foreach ($parts as $part) {
                ...
                switch ($key) {
                    ...
                    case 'modify':
                        $ts =
                          mktime(
                            intval(substr($val, 8, 2)),
                            intval(substr($val, 10, 2)),
                            intval(substr($val, 12, 2)),
                            intval(substr($val, 4, 2)),
                            intval(substr($val, 6, 2)),
                            substr($val, 0, 4)
                          );
                        $stat['ts'] = $ts;
                        break;
            ...
            if (isset($stat['chmod'])) {
                ...
                $stat['read'] =
                  $stat['mime'] == 'directory' ? $read && (($owner && $perm[0][2]) || $perm[1][2] || $perm[2][2]) :
                    $read;
    ...
    {
        if (!($this->connect =
          ftp_connect($this->options['host'], $this->options['port'], $this->options['timeout']))) {
    ...
    protected function parsePermissions($perm, $user = '')
    {
        ...
        return [
          'read'  => $parts[0] == 'd' ?
            $read && (($owner && $parts[3] == 'x') || $parts[6] == 'x' || $parts[9] == 'x') :
            $read,
            ...
            if (!in_array($file, $excludes)) {
                ...
                if (is_link($path)) {
                    ...
                    if (is_dir($path)) {
                        ...
                        $subs =
                          elFinderVolumeFTP::listFilesInDirectory(
                            $path,
                            $omitSymlinks,
                            $prefix . $file . DIRECTORY_SEPARATOR
                          );
                        if ($subs) {
