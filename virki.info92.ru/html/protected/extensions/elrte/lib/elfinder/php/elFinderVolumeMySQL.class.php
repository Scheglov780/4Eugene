...
class elFinderVolumeMySQL extends elFinderVolumeDriver
{
    ...
    {
        return ($stat = $this->stat($path)) ? (!empty($stat['phash']) ? $this->decode($stat['phash']) : $this->root) :
          false;
    }
    ...
    {
        return ($res = $this->query(sprintf('SELECT content FROM %s WHERE id=%d', $this->tbf, $path))) &&
        ($r = $res->fetch_assoc()) ? $r['content'] : false;
    }
    ...
    {
        return $this->query(sprintf('DELETE FROM %s WHERE id=%d AND mime=\'directory\' LIMIT 1', $this->tbf, $path)) &&
