...
class elFinderVolumeDropbox extends elFinderVolumeDriver
{
    ...
    public function __construct()
    {
        ...
        $opts = [
          ...
          'separator'         => '/',
          'PDO_DSN'           => '',
            // if empty use 'sqlite:(metaCachePath|tmbPath)/elFinder_dropbox_db_(hash:dropboxUid+consumerSecret)'
          'PDO_User'          => '',
          'PDO_Pass'          => '',
          'PDO_Options'       => [],
          'PDO_DBName'        => 'dropbox',
          ...
          'tmpPath'           => '',
          'getTmbSize'        => 'large',
            // small: 32x32, medium or s: 64x64, large or m: 128x128, l: 640x480, xl: 1024x768
          'metaCachePath'     => '',
          'metaCacheTime'     => '600',
            // 10m
          'acceptedName'      => '#^[^/\\?*:|"<>]*[^./\\?*:|"<>]$#',
    ...
    {
        $res =
          $this->query('SELECT * FROM sqlite_master WHERE type=\'table\' AND name=\'' . $this->DB_TableName . '\'');
    ...
    private function getDBdat($path)
    {
        if ($res =
          $this->query(
            ...
            $this->DB_TableName .
            ' where path=' .
            $this->DB->quote(strtolower($this->_dirname($path))) .
            ' and fname=' .
            $this->DB->quote(strtolower($this->_basename($path))) .
            ' limit 1'
          )) {
    ...
    private function getHttpResponseHeader($url)
    {
        if (function_exists('curl_exec')) {
            ...
            try {
                ...
                $res = [];
                $res[] =
                  'HTTP/' . $result->getVersion() . ' ' . $result->getStatus() . ' ' . $result->getReasonPhrase();
    ...
    protected function cacheDir($path)
    {
        ...
        $res =
          $this->query('select dat from ' . $this->DB_TableName . ' where path=' . $this->DB->quote(strtolower($path)));
    ...
    protected function deltaCheck($refresh = true)
    {
        $chk = false;
        if (!$refresh &&
          $chk = $this->query('select dat from ' . $this->DB_TableName . ' where path=\'\' and fname=\'\' limit 1')) {
        ...
        try {
            ...
            if ($res =
              $this->query('select dat from ' . $this->DB_TableName . ' where path=\'\' and fname=\'\' limit 1')) {
            ...
            do {
                ...
                if (!empty($_info['reset'])) {
                    ...
                    $this->DB->exec(
                      ...
                      $this->DB_TableName .
                      ' values(\'\', \'\', \'' .
                      serialize(['cursor' => '', 'mtime' => 0]) .
                      '\', 0);'
                    );
                    $this->DB->exec(
                      'insert into ' . $this->DB_TableName . ' values(\'/\', \'\', \'' . serialize([
                        'path'      => '/',
                        'is_dir'    => 1,
                        'mime_type' => '',
                        'bytes'     => 0,
                ...
                foreach ($_info['entries'] as $entry) {
                    ...
                    $_itemTime =
                      strtotime(isset($entry[1]['client_mtime']) ? $entry[1]['client_mtime'] : $entry[1]['modified']);
                    ...
                    if (!$reset && $this->query($sql)) {
                        $this->DB->exec(
                          ...
                          $this->DB_TableName .
                          ' set dat=' .
                          $this->DB->quote(serialize($entry[1])) .
                          ', isdir=' .
                          ($entry[1]['is_dir'] ? 1 : 0) .
                          ' ' .
                          $where
            ...
            foreach ($ptimes as $_p => $_t) {
                if ($praw = $this->getDBdat($_p)) {
                    ...
                    if ($_update) {
                        $pwhere =
                          'where path=' .
                          $this->DB->quote(strtolower($this->_dirname($_p))) .
                          ' and fname=' .
                          $this->DB->quote(strtolower($this->_basename($_p)));
                        $this->DB->exec(
                          'update ' .
                          $this->DB_TableName .
                          ' set dat=' .
                          $this->DB->quote(serialize($praw)) .
                          ' ' .
                          $pwhere
                        );
                    }
            ...
            $this->DB->exec(
              ...
              $this->DB_TableName .
              ' set dat=' .
              $this->DB->quote(serialize(['cursor' => $cursor, 'mtime' => $_SERVER['REQUEST_TIME']])) .
              ' where path=\'\' and fname=\'\''
            );
            if (!$this->DB->commit()) {
                $e = $this->DB->errorInfo();
                return $e[2];
            ...
            foreach ($res as $raw) {
                ...
                if ($stat = $this->parseRaw($raw)) {
                    ...
                    if (!empty($stat['hidden']) ||
                      ($mimes && $stat['mime'] === 'directory') ||
                      !$this->mimeAccepted($stat['mime'], $mimes)) {
                        continue;
                    }
    ...
    protected function init()
    {
        ...
        // make net mount key
        $this->netMountKey = md5(join('-', ['dropbox', $this->options['path']]));
        ...
        if (!$this->oauth) {
            if (defined('ELFINDER_DROPBOX_USE_CURL_PUT')) {
                ...
                if (class_exists('OAuth', false)) {
                    $this->oauth =
                      new Dropbox_OAuth_PHP($this->options['consumerKey'], $this->options['consumerSecret']);
                } else {
                    ...
                    if (class_exists('HTTP_OAuth_Consumer', false)) {
                        $this->oauth =
                          new Dropbox_OAuth_PEAR($this->options['consumerKey'], $this->options['consumerSecret']);
                    }
                }
            }
        }
        ...
        if (empty($this->options['alias'])) {
            $this->options['alias'] =
              ($this->options['path'] === '/') ? 'Dropbox.com' : 'Dropbox' . $this->options['path'];
        }
        ...
        if (!empty($this->options['tmpPath'])) {
            if ((is_dir($this->options['tmpPath']) || mkdir($this->options['tmpPath'])) &&
              is_writable($this->options['tmpPath'])) {
        ...
        if (!empty($this->options['metaCachePath'])) {
            if ((is_dir($this->options['metaCachePath']) || mkdir($this->options['metaCachePath'])) &&
              is_writable($this->options['metaCachePath'])) {
        ...
        if (!$this->options['PDO_DSN']) {
            $this->options['PDO_DSN'] =
              'sqlite:' .
              $this->metaCache .
              DIRECTORY_SEPARATOR .
              '.elFinder_dropbox_db_' .
              md5($this->dropboxUid . $this->options['consumerSecret']);
        }
        // DataBase table name
        ...
        try {
            $this->DB =
              new PDO(
                $this->options['PDO_DSN'],
                $this->options['PDO_User'],
                $this->options['PDO_Pass'],
                $this->options['PDO_Options']
              );
            if (!$this->checkDB()) {
        ...
        if ($raw['is_dir']) {
            $stat['dirs'] =
              (int) (bool) $this->query(
                'select path from ' .
                $this->DB_TableName .
                ' where isdir=1 and path=' .
                $this->DB->quote(strtolower($raw['path']))
              );
        }
    ...
    public function netmountPrepare($options)
    {
        ...
        if ($options['user'] === 'init') {
            ...
            if ($options['pass'] === 'init') {
                ...
                if ($sessionToken = $this->session->get('DropboxTokens')) {
                    // token check
                    try {
                        [, $accessToken, $accessTokenSecret] = $sessionToken;
                ...
                if (!$html) {
                    ...
                    $callback = $options['url']
                      .
                      '?cmd=netmount&protocol=dropbox&host=dropbox.com&user=init&pass=return&node=' .
                      $options['id'] .
                      $cdata;
                    ...
                    $html =
                      '<input id="elf-volumedriver-dropbox-host-btn" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" value="{msg:btnApprove}" type="button" onclick="window.open(\'' .
                      $url .
                      '\')">';
                    $html .= '<script>
        ...
        if ($sessionToken = $this->session->get('DropboxTokens')) {
            [$options['dropboxUid'], $options['accessToken'], $options['accessTokenSecret']] = $sessionToken;
        }
    ...
    public function netunmount($netVolumes, $key)
    {
        ...
        if ($count === 1) {
            $this->DB->exec('drop table ' . $this->DB_TableName);
            foreach (glob(rtrim($this->options['tmbPath'], '\\/') . DIRECTORY_SEPARATOR . $this->tmbPrefix . '*.png') as
                     $tmb) {
                unlink($tmb);
            }
        }
