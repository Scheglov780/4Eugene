...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver
{
    ...
    public function __construct()
    {
        ...
        $this->options['quarantine'] =
          '.quarantine'; // quarantine folder name - required to check archive (must be hidden)
        ...
        $this->options['keepTimestamp'] =
          ['copy', 'move']; // keep timestamp at inner filesystem allowed 'copy', 'move' and 'upload'
    }
    ...
    protected function _inpath($path, $parent)
    {
        ...
        if ($real_path && $real_parent) {
            return $real_path === $real_parent ||
              strpos($real_path, rtrim($real_parent, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR) === 0;
        }
        return false;
    }
    /******************** file/dir content *********************/
                ...
                if ($file->isLink()) {
                    ...
                    if (($dir = $file->isDir()) && $this->options['detectDirIcon']) {
                        ...
                        if ($this->URL && file_exists($favicon)) {
                            $stat['icon'] =
                              $this->URL .
                              str_replace(DIRECTORY_SEPARATOR, '/', substr($favicon, strlen($this->root) + 1));
                        }
                    }
    ...
    protected function _stat($path)
    {
        ...
        if (($dir = is_dir($path)) && $this->options['detectDirIcon']) {
            ...
            if ($this->URL && file_exists($favicon)) {
                $stat['icon'] =
                  $this->URL . str_replace(DIRECTORY_SEPARATOR, '/', substr($favicon, strlen($this->root) + 1));
            }
        }
    ...
    protected function configure()
    {
        ...
        if (!empty($this->options['tmpPath'])) {
            if ((is_dir($this->options['tmpPath']) || mkdir($this->options['tmpPath'], 0755, true)) &&
        ...
        if ($root['read'] && !$this->tmbURL && $this->URL) {
            if (strpos($this->tmbPath, $this->root) === 0) {
                $this->tmbURL =
                  $this->URL . str_replace(DIRECTORY_SEPARATOR, '/', substr($this->tmbPath, strlen($this->root) + 1));
    ...
    protected function doSearch($path, $q, $mimes)
    {
        ...
        try {
            ...
            foreach ($iterator as $key => $node) {
                if ($timeout && ($this->error || $timeout < time())) {
                    !$this->error &&
                    $this->setError(elFinder::ERROR_SEARCH_TIMEOUT, $this->path($this->encode($node->getPath)));
                    break;
                }
                if ($node->isDir()) {
        ...
        if ($match) {
            foreach ($match as $p) {
                if ($timeout && ($this->error || $timeout < time())) {
                    !$this->error &&
                    $this->setError(elFinder::ERROR_SEARCH_TIMEOUT, $this->path($this->encode(dirname($p))));
                    break;
                }
    ...
    protected function init()
    {
        ...
        if (!isset($this->options['systemRoot'])) {
            if ($cwd[0] === $this->separator || $this->root[0] === $this->separator) {
                ...
                if (preg_match('/^([a-zA-Z]:' . preg_quote($this->separator, '/') . ')/', $this->root, $m)) {
                    $this->systemRoot = $m[1];
                } else {
                    if (preg_match('/^([a-zA-Z]:' . preg_quote($this->separator, '/') . ')/', $cwd, $m)) {
                        $this->systemRoot = $m[1];
                    }
                }
            }
     ...
     * If `inotifywait` need full path, Please add `define('ELFINER_INOTIFYWAIT_PATH', '/PATH_TO/inotifywait');` into
     * connector.php
     * @param string $path
     * @param int    $standby
    ...
    public function localFileSystemInotify($path, $standby, $compare)
    {
        ...
        $cmd =
          ...
          ' -t ' .
          $standby .
          ' -e moved_to,moved_from,move,close_write,delete,delete_self';
