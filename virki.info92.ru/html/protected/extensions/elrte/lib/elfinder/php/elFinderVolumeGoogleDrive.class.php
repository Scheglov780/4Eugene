...
class elFinderVolumeGoogleDrive extends elFinderVolumeDriver
{
    ...
    {
        [, $itemId] = $this->_gd_splitPath($path);
        if (!$fields) {
    ...
    {
        [, $basename] = $this->_gd_splitPath($path);
    ...
    protected function _copy($source, $targetDir, $name)
    {
        ...
        try {
            ...
            //Set the Parent id
            [, $parentId] = $this->_gd_splitPath($targetDir);
            $file->setParents([$parentId]);

            [, $srcId] = $this->_gd_splitPath($source);
    ...
    {
        [, , $parent] = $this->_gd_splitPath($path);
        ...
        if ($raw['mimeType'] === self::DIRMIME) {
            ...
            if ($this->options['useGoogleTmb']) {
                if (isset($raw['thumbnailLink'])) {
                    if ($published) {
                        $stat['tmb'] =
                          'drive.google.com/thumbnail?authuser=0&sz=s' .
                          $this->options['tmbSize'] .
                          '&id=' .
                          $raw['id'];
                    } else {
    ...
    protected function _gd_publish($path)
    {
        if ($file = $this->_gd_getFile($path)) {
            ...
            try {
                if ($this->service->permissions->create(
                  $file->getId(),
                  new \Google_Service_Drive_Permission($this->options['publishPermission'])
                )) {
                    return true;
                }
            } catch (Exception $e) {
    ...
    protected function _mkdir($path, $name)
    {
        ...
        [$parentId, , $parent] = $this->_gd_splitPath($path);
    ...
    {
        [$removeParents, $itemId] = $this->_gd_splitPath($source);
        ...
        try {
            ...
            [, $addParents] = $this->_gd_splitPath($targetDir);
    ...
    protected function _save($fp, $path, $name, $stat)
    {
        ...
        [$parentId, $itemId, $parent] = $this->_gd_splitPath($path);
        if ($name === '') {
            $stat['iid'] = $itemId;
        }
    ...
    protected function _unlink($path)
    {
        try {
            ...
            [$pid, $itemId] = $this->_gd_splitPath($path);
    ...
    protected function copy($src, $dst, $name)
    {
        ...
        if ($res['mimeType'] == self::DIRMIME) {
            ...
            if ($newDir) {
                [, $itemId] = $this->_gd_splitPath($newDir);
                [, $srcId] = $this->_gd_splitPath($src);
                ...
                $res = $this->_gd_query($opts);
                foreach ($res as $raw) {
                    $raw['mimeType'] == self::DIRMIME ? $this->copy($src . '/' . $raw['id'], $path, $raw['name']) :
                      $this->_copy($src . '/' . $raw['id'], $path, $raw['name']);
                }
    ...
    protected function init()
    {
            ...
            if ($this->options['refresh_token']) {
                ...
                if ($tmp) {
                    $aTokenFile =
                      $tmp .
                      DIRECTORY_SEPARATOR .
                      md5($this->options['client_id'] . $this->options['refresh_token']) .
                      '.gtoken';
            ...
            if (!$this->service) {
                ...
                if ($client->isAccessTokenExpired()) {
                    ...
                    if (!empty($this->options['netkey'])) {
                        elFinder::$instance->updateNetVolumeOption(
                          $this->options['netkey'],
                          'access_token',
                          $access_token
                        );
        ...
        $this->options['root'] == '' ? $this->options['root'] = $this->_gd_getNameByPath('root') :
          $this->options['root'];

        if (empty($this->options['alias'])) {
            $this->options['alias'] =
              ($this->options['path'] === '/') ? $this->options['root'] :
                sprintf($this->options['gdAlias'], $this->_gd_getNameByPath($this->options['path']));
        }
    ...
    {
        [$parentId, $name] = $this->_gd_splitPath($path);
        $opts = [
          'q'      => sprintf('trashed=false and "%s" in parents and name="%s"', $parentId, $name),
          'fields' => self::FETCHFIELDS_LIST,
        ];
        ...
        try {
            ...
            if ($options['user'] === 'init') {
                ...
                if (!$aToken && empty($_GET['code'])) {
                    ...
                    $html =
                      '<input id="elf-volumedriver-googledrive-host-btn" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" value="{msg:btnApprove}" type="button" onclick="window.open(\'' .
                      $url .
                      '\')">';
                    $html .= '<script>
                    ...
                    if (empty($options['pass']) && $options['host'] !== '1') {
                        ...
                        $out = [
                          'node' => $options['id'],
                          'json' => '{"protocol": "googledrive", "mode": "makebtn", "body" : "' .
                            str_replace($html, '"', '\\"') .
                            '", "error" : "' .
                    ...
                    $json =
                      '{"protocol": "googledrive", "mode": "done", "folders": ' .
                      $folders .
                      ', "expires": ' .
                      $expires .
                      '}';
    ...
    public function netunmount($netVolumes, $key)
    {
        if (!$this->options['useGoogleTmb']) {
            if ($tmbs =
              glob(rtrim($this->options['tmbPath'], '\\/') . DIRECTORY_SEPARATOR . $this->netMountKey . '*.png')) {
