...
class elFinderVolumeOneDrive extends elFinderVolumeDriver
{
    ...
    {
        [, $basename] = $this->_od_splitPath($path);
    ...
    protected function _copy($source, $targetDir, $name)
    {
        ...
        try {
            //Set the Parent id
            [, $parentId] = $this->_od_splitPath($targetDir);
            [, $itemId] = $this->_od_splitPath($source);
    ...
    {
        [, , $dirname] = $this->_od_splitPath($path);
    ...
    {
        if ($mode === 'rb' || $mode === 'r') {
            [, $itemId] = $this->_od_splitPath($path);
            $data = [
    ...
    protected function _mkdir($path, $name)
    {
        ...
        [$parentId] = $this->_od_splitPath($namePath);
    ...
    {
        try {
            [, $targetParentId] = $this->_od_splitPath($targetDir);
            [$sourceParentId, $itemId, $srcParent] = $this->_od_splitPath($source);
    ...
    protected function _save($fp, $path, $name, $stat)
    {
        $itemId = '';
        if ($name === '') {
            [$parentId, $itemId, $parent] = $this->_od_splitPath($path);
        } else {
            if ($stat) {
                if (isset($stat['name'])) {
                    $name = $stat['name'];
            ...
            [, $parentId] = $this->_od_splitPath($path);
            $parent = $path;
        }
    ...
    protected function init()
    {
        ...
        if (empty($options['netkey'])) {
            // make net mount key
            $_tokenKey =
              isset($this->token->data->refresh_token) ? $this->token->data->refresh_token :
    ...
    {
        [$pid, $name] = $this->_od_splitPath($path);
            ...
            try {
                ...
                if ($result) {
                    ...
                    if (isset($result->link)) {
                        [, $res] = explode('?', $result->link->webUrl);
            ...
            if ($options['user'] === 'init') {
                ...
                if (!$result) {
                    ...
                    $callback = $options['url']
                      .
                      '?cmd=netmount&protocol=onedrive&host=onedrive.com&user=init&pass=return&node=' .
                      $options['id'] .
                      $cdata;
                    ...
                    $html =
                      '<input id="elf-volumedriver-onedrive-host-btn" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" value="{msg:btnApprove}" type="button" onclick="window.open(\'' .
                      $url .
                      '\')">';
                    $html .= '<script>
                    ...
                    $json =
                      '{"protocol": "onedrive", "mode": "done", "folders": ' .
                      $folders .
                      ', "expires": ' .
                      $expires .
                      '}';
