...
class elFinderVolumeBox extends elFinderVolumeDriver
{
    ...
    {
        [, $basename] = $this->_bd_splitPath($path);
    ...
    protected function _bd_getRawItem($path)
    {
        ...
        [, $itemId] = $this->_bd_splitPath($path);
    ...
    protected function _copy($source, $targetDir, $name)
    {
        try {
            //Set the Parent id
            [, $parentId] = $this->_bd_splitPath($targetDir);
            [$srcPid, $srcId] = $this->_bd_splitPath($source);
    ...
    {
        [, , $dirname] = $this->_bd_splitPath($path);
    ...
    {
        try {
            [, $parentId] = $this->_bd_splitPath($path);
    ...
    protected function _move($source, $targetDir, $name)
    {
        try {
            ...
            [, $parentId] = $this->_bd_splitPath($targetDir);
            [, $itemId] = $this->_bd_splitPath($source);
    ...
    protected function _save($fp, $path, $name, $stat)
    {
        $itemId = '';
        if ($name === '') {
            [$parentId, $itemId, $parent] = $this->_bd_splitPath($path);
        } else {
            if ($stat) {
                if (isset($stat['name'])) {
                    $name = $stat['name'];
    ...
    protected function getSharedWebContentLink($raw)
    {
        ...
        if ($raw->shared_link->url && ($fType == 'image' || $fType == 'video' || $fType == 'audio')) {
            if ($fExtension == 'jpg' && $fType == 'image') {
                $url =
                  'https://app.box.com/representation/file_version_' .
                  $raw->file_version->id .
                  '/image_2048_' .
                  $fExtension .
                  '/1.' .
                ...
                $url =
                  'https://app.box.com/index.php?rm=preview_stream&amp&file_version_' .
                  $raw->file_version->id .
                  '/audio/mpeg:' .
                  $raw->name .
                  '&shared_name=' .
    ...
    protected function init()
    {
        ...
        if (empty($this->options['alias'])) {
            [, $itemId] = $this->_bd_splitPath($this->options['path']);
    ...
    {
        [$pid, $name, $parent] = $this->_bd_splitPath($path);
        ...
        try {
            ...
            if ($options['user'] === 'init') {
                ...
                if (!$result) {
                    ...
                    $html =
                      '<input id="elf-volumedriver-box-host-btn" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" value="{msg:btnApprove}" type="button" onclick="window.open(\'' .
                      $url .
                      '\')">';
                    $html .= '<script>
                    ...
                    $json =
                      '{"protocol": "box", "mode": "done", "folders": ' . $folders . ', "expires": ' . $expires . '}';
                    $html = 'Box.com';
                    $html .= '<script>
