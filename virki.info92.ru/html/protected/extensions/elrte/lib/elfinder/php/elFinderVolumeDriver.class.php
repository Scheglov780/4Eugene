...
abstract class elFinderVolumeDriver
{
    ...
    {
        return (!$this->encoding) ? $this->_abspath($path) :
          $this->convEncOut($this->_abspath($this->convEncIn($path)));
    }
    ...
    {
        return (!$this->encoding) ? $this->_basename($path) :
          $this->convEncOut($this->_basename($this->convEncIn($path)));
    }
        ...
        if ((!$checkTmbPath || $this->tmbPathWritable)
          && (!$this->tmbPath || strpos($path, $this->tmbPath) === false) // do not create thumnbnail for thumnbnail
            ...
            return $this->imgLib
              ...
              &&
              ($this->imgLib == 'gd' ?
                in_array($stat['mime'], ['image/jpeg', 'image/png', 'image/gif', 'image/x-ms-bmp']) : true);
        }
        return false;
    }
    ...
    protected function configure()
    {
        ...
        if (($type === 'imagick' || $type === 'auto') && extension_loaded('imagick')) {
            ...
            if (($type === 'gd' || $type === 'auto') && function_exists('gd_info')) {
                $this->imgLib = 'gd';
            } else {
    ...
    protected function copy($src, $dst, $name)
    {
        ...
        if ($srcStat['mime'] === 'directory') {
            ...
            if (($testStat && $testStat['mime'] != 'directory') ||
              (!$testStat && !$testStat = $this->mkdir($this->encode($dst), $name))) {
        ...
        if ($res =
          $this->convEncOut($this->_copy($this->convEncIn($src), $this->convEncIn($dst), $this->convEncIn($name)))) {
    ...
    protected function copyFrom($volume, $src, $destination, $name)
    {
        ...
        if ($source['mime'] == 'directory') {
            ...
            if (($test && $test['mime'] != 'directory') ||
              (!$test && !$test = $this->mkdir($this->encode($destination), $name))) {
            ...
            if (!$this->allowPutMime($stat['mime']) ||
              ($mimeByName && $mimeByName !== 'unknown' && !$this->allowPutMime($mimeByName))) {
    ...
    protected function createTmb($path, $stat)
    {
        ...
        if ($s[0] <= $tmbSize && $s[1] <= $tmbSize) {
            $result =
              $this->imgSquareFit($tmb, $tmbSize, $tmbSize, 'center', 'middle', $this->options['tmbBgColor'], 'png');
        } else {
            ...
            if ($this->options['tmbCrop']) {
                ...
                if (!(($s[0] > $tmbSize && $s[1] <= $tmbSize) || ($s[0] <= $tmbSize && $s[1] > $tmbSize)) ||
    ...
    {
        return (!$this->encoding) ? $this->_dirname($path) :
          $this->convEncOut($this->_dirname($this->convEncIn($path)));
    }
    ...
    {
        return (!$this->encoding) ? $this->_fclose($fp, $path) :
          $this->convEncOut($this->_fclose($fp, $this->convEncIn($path)));
    }
    ...
    {
        return (!$this->encoding) ? $this->_fopen($path, $mode) :
          $this->convEncOut($this->_fopen($this->convEncIn($path), $mode));
    }
        ...
        if (class_exists('ZipArchive', false)) {
            if (empty($arcs['create']['application/zip'])) {
                $arcs['create']['application/zip'] =
                  ['cmd' => 'phpfunction', 'argc' => ['self', 'zipArchiveZip'], 'ext' => 'zip'];
            }
            if (empty($arcs['extract']['application/zip'])) {
                $arcs['extract']['application/zip'] =
                  ['cmd' => 'phpfunction', 'argc' => ['self', 'zipArchiveUnzip'], 'ext' => 'zip'];
            }
        }
    ...
    protected function getFullPath($path, $base)
    {
        ...
        if ($base[0] === $separator &&
          strpos($base, '0', strlen($systemroot)) !== $systemroot) { //Alexys was 0 as integer in strpos
        ...
        // normalize `/../`
        $normreg =
          '#(' . $sepquoted . ')[^' . $sepquoted . ']+' . $sepquoted . '\.\.' . $sepquoted . '#'; // '#(/)[^\/]+/\.\./#'
    ...
    protected function imgCrop($path, $width, $height, $x, $y, $destformat = null, $jpgQuality = null)
    {
        ...
        switch ($this->imgLib) {
            ...
            case 'convert':
                ...
                $cmd =
                  sprintf(
                    'convert %s%s%s -crop %dx%d+%d+%d%s %s',
                    $quotedPath,
                    $coalesce,
                    $jpgQuality,
                    $width,
      ...
      $options = []
        ...
        switch ($this->imgLib) {
            ...
            case 'convert':
                ...
                $cmd =
                  sprintf(
                    'convert %s%s%s %s -geometry %dx%d! %s %s',
                    $quotedPath,
                    $coalesce,
                    $jpgQuality,
                    $filter,
    ...
    protected function imgRotate($path, $degree, $bgcolor = '#ffffff', $destformat = null, $jpgQuality = null)
    {
        ...
        switch ($this->imgLib) {
            ...
            case 'convert':
                ...
                $cmd =
                  sprintf(
                    'convert %s%s%s -background "%s" -rotate %d%s %s',
                    $quotedPath,
                    $coalesce,
                    $jpgQuality,
                    $bgcolor,
      ...
      $jpgQuality = null
        ...
        switch ($this->imgLib) {
            ...
            case 'convert':
                ...
                $cmd =
                  sprintf(
                    'convert -size %dx%d "xc:%s" png:- | convert%s%s png:-  %s -geometry +%d+%d -compose over -composite%s %s',
                    $width,
                    $height,
                    $bgcolor,
                    $coalesce,
    ...
    {
        return (!$this->encoding) ? $this->_inpath($path, $parent) :
          $this->convEncOut($this->_inpath($this->convEncIn($path), $this->convEncIn($parent)));
    }
        ...
        if (!empty($this->ARGS['reload'])) {
            if ($ARGtarget === '') {
                $ARGtarget = isset($this->ARGS['target']) ? $this->ARGS['target']
                  :
                  ((isset($this->ARGS['targets']) &&
                    is_array($this->ARGS['targets']) &&
                    count($this->ARGS['targets']) === 1) ?
                    $this->ARGS['targets'][0] : '');
            }
            if ($ARGtarget !== '') {
    ...
    {
        return (!$this->encoding) ? $this->_joinPath($dir, $name) :
          $this->convEncOut($this->_joinPath($this->convEncIn($dir), $this->convEncIn($name)));
    }
    ...
    protected function mimetype($path, $name = '')
    {
        ...
        if (is_readable($path)) {
            if ($this->mimeDetect == 'finfo') {
                ...
                if ($this->mimeDetect == 'mime_content_type') {
    ...
    protected function move($src, $dst, $name)
    {
        ...
        if ($res =
          $this->convEncOut($this->_move($this->convEncIn($src), $this->convEncIn($dst), $this->convEncIn($name)))) {
            $this->removed[] = $stat;
    ...
    {
        return (!$this->encoding) ? $this->_normpath($path) :
          $this->convEncOut($this->_normpath($this->convEncIn($path)));
    }
    ...
    {
        return (!$this->encoding) ? $this->_relpath($path) :
          $this->convEncOut($this->_relpath($this->convEncIn($path)));
    }
    ...
    {
        return (!$this->encoding) ? $this->_save($fp, $dir, $name, $stat) :
          $this->convEncOut(
            $this->_save($fp, $this->convEncIn($dir), $this->convEncIn($name), $this->convEncIn($stat))
          );
    }
    ...
    {
        return (!$this->encoding) ? $this->_scandir($path) :
          $this->convEncOut($this->_scandir($this->convEncIn($path)));
    }
    ...
    protected function subdirsCE($path)
    {
        ...
        $hasdir =
          (bool) ((!$this->encoding) ? $this->_subdirs($path) :
            $this->convEncOut($this->_subdirs($this->convEncIn($path))));
        if (isset($this->sessionCache['subdirs'])) {
            $this->sessionCache['subdirs'][$path] = $hasdir;
        }
        return $hasdir;
    ...
    {
        return (!$this->encoding) ? $this->_symlink($source, $targetDir, $name) :
          $this->convEncOut(
            $this->_symlink($this->convEncIn($source), $this->convEncIn($targetDir), $this->convEncIn($name))
          );
    }
    ...
    protected function updateCache($path, $stat)
    {
        ...
        if ($this->options['utf8fix'] && $this->options['utf8patterns'] && $this->options['utf8replace']) {
            $stat['name'] =
              json_decode(str_replace($this->options['utf8patterns'], $this->options['utf8replace'], $jeName));
        }
        ...
        if ($stat['read'] && empty($stat['hidden'])) {
            ...
            if ($isDir) {
                ...
                if ($parent) {
                    if (isset($this->sessionCache['subdirs'])) {
                        $this->sessionCache['subdirs'][$parent] = true;
                    }
                }
            ...
            if (!isset($stat['url']) && $this->URL && $this->encoding) {
                ...
                $stat['url'] =
                  ...
                  str_replace(
                    '%2F',
                    '/',
                    rawurlencode((substr(PHP_OS, 0, 3) === 'WIN') ? $_path : $this->convEncIn($_path, true))
                  );
            }
        } else {
            if ($isDir) {
    ...
    public function archive($hashes, $mime, $name = '')
    {
        ...
        if ($name === '') {
            ...
            $name =
              str_replace(['/', '\\'], '_', preg_replace('/\.' . preg_quote($archiver['ext'], '/') . '$/i', '', $name));
        }
        $name .= '.' . $archiver['ext'];
        ...
        return ($path =
          $this->convEncOut(
            $this->_archive($this->convEncIn($dir), $this->convEncIn($files), $this->convEncIn($name), $archiver)
    ...
    {
        return (!$this->encoding) ? $var :
          $this->convEnc($var, 'UTF-8', $this->encoding, $this->options['locale'], $restoreLocale, $unknown);
    }
    ...
    {
        return (!$this->encoding) ? $var :
          $this->convEnc($var, $this->encoding, 'UTF-8', $this->options['locale'], $restoreLocale, $unknown);
    }
    ...
    public function ffmpegToImg($file, $stat)
    {
        ...
        if (rename($file, $tmp)) {
            ...
            $cmd =
              sprintf(
                'ffmpeg -i %s -ss 00:00:%.3f -vframes 1 -f image2 %s',
                escapeshellarg($tmp),
                $ss,
                escapeshellarg($file)
              );
    ...
    public function getTempPath()
    {
        ...
        if (isset($this->tmpPath) && $this->tmpPath && is_writable($this->tmpPath)) {
            ...
            if (isset($this->tmp) && $this->tmp && is_writable($this->tmp)) {
                $tempPath = $this->tmp;
            } else {
                if (function_exists('sys_get_temp_dir')) {
                    $tempPath = sys_get_temp_dir();
                } else {
                    if (isset($this->tmbPath) && $this->tmbPath && is_writable($this->tmbPath)) {
    ...
    public function mkdir($dsthash, $name)
    {
        ...
        if ($mkpath && isset($this->sessionCache['subdirs'])) {
            $this->sessionCache['subdirs'][$path] = true;
            $this->sessionCache['subdirs'][$mkpath] = false;
        }
    ...
    public function mkfile($dst, $name)
    {
        ...
        $this->clearcache();
        return ($path = $this->convEncOut($this->_mkfile($this->convEncIn($path), $this->convEncIn($name)))) ?
          $this->stat($path) : false;
    }
    ...
    public function mount(array $opts)
    {
        ...
        if (!$this->session) {
            return $this->setError(
              'Session wrapper dose not set. Need to `$volume->setSession(elFinderSessionInterface);` before mount.'
            );
        }
        ...
        $this->id =
          $this->driverId . (!empty($this->options['id']) ? $this->options['id'] : elFinder::$volumesCnt++) . '_';
        ...
        if (is_string($this->options['uploadOrder'])) { // telephat_mode on, compatibility with 1.x
            ...
            $this->uploadOrder =
              !empty($this->options['uploadOrder']) ? $this->options['uploadOrder'] : ['deny', 'allow'];
        }
        ...
        $this->nameValidator =
          ...
          (is_string($this->options['acceptedName']) || is_callable($this->options['acceptedName']))
        ...
        if ($this->options['syncMinMs'] !== 0) {
            $this->options['syncMinMs'] =
              max(
                $this->options[$this->options['syncChkAsTs'] ? 'tsPlSleep' : 'lsPlSleep'] * 1000,
                intval($this->options['syncMinMs'])
              );
        }
    ...
    public function options($hash)
    {
        ...
        $opts = [
          ...
          'archivers'       => [
            'create'    => $create,
            'extract'   => isset($this->archivers['extract']) && is_array($this->archivers['extract']) ?
              array_keys($this->archivers['extract']) : [],
            'createext' => $createext,
          ],
          'uiCmdMap'        => (isset($this->options['uiCmdMap']) && is_array($this->options['uiCmdMap'])) ?
            $this->options['uiCmdMap'] : [],
    ...
    public function paste($volume, $src, $dst, $rmSrc = false, $hashes = [])
    {
        ...
        if (($test = $volume->closest($src, $rmSrc ? 'locked' : 'read', $rmSrc))) {
            return $rmSrc
              ...
              $this->setError(
                $err,
                $errpath,
                !empty($file['thash']) ? elFinder::ERROR_PERM_DENIED : elFinder::ERROR_MKOUTLINK
              );
        }
        ...
        if ($stat && $stat['name'] === $name) { // file exists and check filename for item ID based filesystem
            if ($this->options['copyOverwrite']) {
                ...
                if ($volume === $this &&
                  ((isset($file['target']) && $test == $file['target']) || $test == $this->decode($src))) {
    ...
    public function putContents($hash, $content)
    {
        ...
        $this->clearcache();
        return $this->convEncOut($this->_filePutContents($this->convEncIn($path), $content)) ? $this->stat($path) :
          false;
    }
    ...
    public function rename($hash, $name)
    {
        ...
        if ($path =
          $this->convEncOut($this->_move($this->convEncIn($path), $this->convEncIn($dir), $this->convEncIn($name)))) {
            $this->clearcache();
            return $this->stat($path);
        }
        return false;
    ...
    public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0, $jpgQuality = null)
    {
        ...
        switch ($mode) {
            ...
            case 'rotate':
                $result =
                  $this->imgRotate($work_path, $degree, ($bg ? $bg : $this->options['bgColorFb']), null, $jpgQuality);
                break;
     ...
     * @param string $exclude dir hash which subfolders must be exluded from result, required to not get stat twice on
     *                        cwd subfolders
     * @return array|false
     * @author Dmitry (dio) Levashov
     **/
    public function tree($hash = '', $deep = 0, $exclude = '')
    {
        ...
        $dirs =
          $this->gettree($path, $deep > 0 ? $deep - 1 : $this->treeDeep - 1, $exclude ? $this->decode($exclude) : null);
    ...
    public function zipdl($hashes)
    {
        ...
        if ($cnt > 1) {
            for ($i = 1; $i < $cnt; $i++) {
                if ($dirname !==
                  dirname(str_replace($this->separator, DIRECTORY_SEPARATOR, $this->path($hashes[$i])))) {
                    $mixed = true;
                    break;
                }
            }
    ...
    static protected function mimetypeInternalDetect($path = '')
    {
        ...
        return ($ext && isset(elFinderVolumeDriver::$mimetypes[$ext])) ? elFinderVolumeDriver::$mimetypes[$ext] :
          'unknown';
    }
