...
class elFinder
{
    ...
    const ERROR_NOT_UTF8_CONTENT = 'errNotUTF8Content';     // 'File not found.'
    const ERROR_OPEN = 'errOpen'; // 'Target folder "$1" not found.'
    const ERROR_PERM_DENIED = 'errPerm';
    const ERROR_REAUTH_REQUIRE = 'errReauthRequire';
    const ERROR_RENAME = 'errRename';
    const ERROR_REPLACE = 'errReplace';        // '"$1" is locked and can not be renamed, moved or removed.'
    const ERROR_RESIZE = 'errResize';        // 'File named "$1" already exists.'
    const ERROR_RESIZESIZE = 'errResizeSize';       // 'Invalid file name.'
    ...
    const ERROR_UNSUPPORT_TYPE = 'errUsupportType';          // 'Unable to replace "$1".'
    const ERROR_UPLOAD = 'errUpload';               // 'Unable to remove "$1".'
    const ERROR_UPLOAD_FILE = 'errUploadFile';            // 'Unable remove source file(s)'
    const ERROR_UPLOAD_FILE_MIME = 'errUploadMime';        // 'Unable to create a link to outside the volume root.'
    const ERROR_UPLOAD_FILE_SIZE = 'errUploadFileSize';           // 'Upload error.'
    const ERROR_UPLOAD_NO_FILES = 'errUploadNoFiles';       // 'Unable to upload "$1".'
    const ERROR_UPLOAD_TEMP = 'errUploadTemp';    // 'No files found for upload.'
    const ERROR_UPLOAD_TOTAL_SIZE = 'errUploadTotalSize';  // 'Data exceeds the maximum allowed size.'
    const ERROR_UPLOAD_TRANSFER = 'errUploadTransfer';   // 'File exceeds maximum allowed size.'
    ...
    public function __construct($opts)
    {
        ...
        if (!empty($opts['session']) && $opts['session'] instanceof elFinderSessionInterface) {
            ...
            $sessionOpts = [
              ...
              'keys'         => [
                ...
                'netvolume' => !empty($opts['netVolumesSessionKey']) ? $opts['netVolumesSessionKey'] :
                  'elFinderNetVolumes',
              ],
            ];
        ...
        if (isset($_SERVER['HTTP_X_ELFINDER_VOLUMESCNTSTART']) &&
          ($volumesCntStart = intval($_SERVER['HTTP_X_ELFINDER_VOLUMESCNTSTART']))) {
        ...
        $this->optionsNetVolumes =
          (isset($opts['optionsNetVolumes']) && is_array($opts['optionsNetVolumes'])) ? $opts['optionsNetVolumes'] : [];

        // deprecated settings
        $this->netVolumesSessionKey =
          !empty($opts['netVolumesSessionKey']) ? $opts['netVolumesSessionKey'] : 'elFinderNetVolumes';
                                ...
                                if (strcasecmp($_domain, 'plugin') === 0) {
                                    if ($plugin =
                                        $this->getPluginInstance(
                                          $_name,
                                          isset($opts['plugin'][$_name]) ? $opts['plugin'][$_name] : []
                                        )
        ...
        foreach ($opts['roots'] as $i => $o) {
            ...
            if (class_exists($class)) {
                ...
                try {
                    if ($this->maxArcFilesSize &&
                      (empty($o['maxArcFilesSize']) || $this->maxArcFilesSize < $o['maxArcFilesSize'])) {
     ...
     * @var string URL
     */
    protected $callbackWindowURL = '';   // '"$1" transfer error.'
    /**
     ...
     **/
    protected $default = null;       // Object "$1" already exists at this location and can not be replaced with object of another type.
    /**
     * Commands listeners
     * @var array
     **/
     ...
     * @var array
     */
    public static $netDrivers = [];    // 'Timed out while searching "$1". Search result is partial.'
    /**
     * Errors from PHP
     * @var array
     **/
    public static $phpErrors = [];  // 'Re-authorization is required.'
    /**
     * elFinder global sessionCacheKey
     * @deprecated
     * @var string
     */
    public static $sessionCacheKey = ''; // 'Max number of selectable items is $1.'
    /**
    ...
    protected function callback($args)
    {
        ...
        if ($done || !$this->callbackWindowURL) {
            ...
            $out =
              '<!DOCTYPE html><html><head><meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><script>' .
              $script .
              '</script></head><body><a href="#" onlick="window.close();return false;">Close this window</a></body></html>';
     ...
     * @return array|false      array('stats' => array([stat of maked directory]), 'hashes' => array('[path]' =>
     *                          '[hash]'))
     * @author Naoki Sawada
     **/
    protected function ensureDirsRecursively($volume, $target, $dirs, $path = '')
    {
        ...
        foreach ($dirs as $name => $sub) {
            $name = (string) $name;
            if ((($parent = $volume->realpath($target)) && ($dir = $volume->dir($volume->getHash($parent, $name)))) ||
    ...
    protected function extract($args)
    {
        ...
        return ($file = $volume->extract($target, $makedir))
          ? ['added' => isset($file['read']) ? [$file] : $file]
    ...
    protected function file($args)
    {
        ...
        if (strpos($filenameEncoded, '%') === false) { // ASCII only
            ...
            if (preg_match('/MSIE [4-8]/', $ua)) { // IE < 9 do not support RFC 6266 (RFC 2231/RFC 5987)
              ...
              preg_match('#Version/[3-5]#', $ua)) { // Safari < 6
    ...
    protected function get_remote_contents(&$url, $timeout = 30, $redirect_max = 5, $ua = 'Mozilla/5.0', $fp = null)
    {
        ...
        $method =
          (function_exists('curl_exec') && !ini_get('safe_mode') && !ini_get('open_basedir')) ? 'curl_get_contents' :
            'fsock_get_contents';
    ...
    protected function mkdir($args)
    {
        ...
        if ($dirs) {
            ...
            return ($res = $this->ensureDirsRecursively($volume, $target, $mkdirs)) === false
              ...
              : ['added' => $res['stats'], 'hashes' => $res['hashes']];
        } else {
        ...
        foreach ($targets as $target) {
            ...
            if ($renames) {
                ...
                if (isset($renames[$file['name']])) {
                    ...
                    $rnres =
                      $this->rename(
                        ['target' => $hash, 'name' => $dstVolume->uniqueName($dir, $file['name'], $suffix, true, 0)]
                      );
                ...
                foreach ($args['upload'] as $i => $url) {
                    ...
                    if ($args['chunk']) {
                        if ($url === 'chunkfail' && $args['mimes'] === 'chunkfail') {
                            ...
                            if (preg_match('/^(.+)(\.\d+_(\d+))\.part$/s', $chunk, $m)) {
                                $result['warning'] =
                                  $this->error(self::ERROR_UPLOAD_FILE, $m[1], self::ERROR_UPLOAD_TRANSFER);
                            }
                            return $result;
                        } else {
                    ...
                    if (substr($url, 0, 5) === 'data:') {
                        [$data, $args['name'][$i]] = $this->parse_data_scheme($url, $extTable);
                    } else {
                    ...
                    if ($data) {
                        ...
                        if ($_name) {
                            ...
                            if ((is_resource($data) && fclose($data)) || file_put_contents($tmpfname, $data)) {
                                ...
                                [$_a, $_b] = array_pad(explode('.', $_name, 2), 2, '');
                                if ($_b === '') {
                                    if ($_ext) {
        ...
        foreach ($files['name'] as $i => $name) {
            if (($error = $files['error'][$i]) > 0) {
                $result['warning'] =
                  $this->error(
                    ...
                    $name,
                    $error == UPLOAD_ERR_INI_SIZE || $error == UPLOAD_ERR_FORM_SIZE ? self::ERROR_UPLOAD_FILE_SIZE :
            ...
            if ($path !== '' && $path !== $target) {
                ...
                if (isset($renames[$name])) {
                    ...
                    $rnres =
                      $this->rename(['target' => $hash, 'name' => $volume->uniqueName($dir, $name, $suffix, true, 0)]);
                    if (!empty($rnres['error'])) {
                        $result['warning'] = $rnres['error'];
                        break;
                    }
            ...
            if (($volume = $this->volume($targets[0])) !== false) {
                if ($dlres = $volume->zipdl($targets)) {
                    ...
                    register_shutdown_function(function ($f) {
                        connection_status() && is_file($f) && unlink($f);
                    }, $path);
                    if (count($targets) === 1) {
