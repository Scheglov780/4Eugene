...
class elFinderVolumeFlysystemGoogleDriveNetmount extends \Hypweb\elFinderFlysystemDriverExt\Driver
{
    ...
    public function mount(array $opts)
    {
              ...
              md5(
                join(
                  ...
                  [
                    'googledrive',
                    $opts['path'],
                    (isset($opts['access_token']['refresh_token']) ? $opts['access_token']['refresh_token'] :
        ...
        if ($this->options['gdCacheDir'] && is_writeable($this->options['gdCacheDir'])) {
            if ($this->options['gdCacheExpire']) {
                $opts['fscache'] =
                  new elFinderVolumeFlysystemGoogleDriveCache(
                    new Local($this->options['gdCacheDir']),
                    $this->options['gdCachePrefix'] . $this->netMountKey,
                    $this->options['gdCacheExpire']
                  );
            }
        }
        if ($opts['fscache']) {
        ...
        if ($res = parent::mount($opts)) {
            ...
            if ($creds) {
                ...
                $netVolumes[$this->netMountKey]['access_token'] =
                  array_merge($netVolumes[$this->netMountKey]['access_token'], $creds);
    ...
    {
        $cache =
          $this->options['gdCacheDir'] . DIRECTORY_SEPARATOR . $this->options['gdCachePrefix'] . $this->netMountKey;
