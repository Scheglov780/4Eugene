	...
	$.fn.elSelect = function(o) {
		...
		self.empty()
			...
			.click(function(e) {
				...
				if (navigator.userAgent.match(/msie|trident/i) && !ieWidth) { 
					list.children().each(function() {
	...
	.bind('keydown', function(e) {
		if ((e.metaKey || e.ctrlKey) && e.keyCode == 65) {
			...
			if (self.dom.selfOrParent(n, /^PRE$/)) {
				...
				return false;
			} else if (navigator.userAgent.match(/safari/i) && e.shiftKey) {
		...
		if (e.keyCode == 32 && navigator.userAgent.match(/opera/i)) {
			self.selection.insertNode(self.doc.createTextNode(" "));
			return false
		}
	})
...
elRTE.prototype.dom = function(rte) {
	...
	* @param  RegExp||String  pf   фильтр условия для родителя
	...
	this.cleanNode = function(n) {
		...
		if (navigator.userAgent.match(/safari/i) && $n.hasClass('Apple-span')) {
...
(function($) {
	...
	elRTE.prototype.filter.prototype.rules = {
		...
		replace : function(html) {
			...
			html = html
                ...
                .replace(this.phpRegExp, "<!-- ELRTE_COMMENT$1 -->")
	...
	elRTE.prototype.filter.prototype.chains = {
		wysiwyg         : [],//Alexys['replace', 'clean', 'allowedTags', 'deniedTags', 'compactStyles'],
		source          : [],//Alexys['clean', 'allowedTags', 'restore', 'compactStyles', 'xhtmlTags'],
		paste           : [],//Alexys['clean', 'allowedTags', 'cleanPaste', 'replace', 'deniedTags', 'compactStyles'],
		wysiwyg2wysiwyg : [],//Alexys['clean', 'allowedTags', 'restore', 'replace', 'deniedTags', 'compactStyles'],
		source2source   : []//Alexys['clean', 'allowedTags', 'replace', 'deniedTags', 'restore', 'compactStyles', 'xhtmlTags']
	}
...
elRTE.prototype.options   = {
	doctype         : '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">',//Alexy '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">',
    cssClass        : 'el-rte',
	cssfiles        : [],
	height          : null,
	resizable       : true,
	...
	toolbars    : {
        ...
        yii     : ['copypaste', 'undoredo', 'elfinder', 'style', 'alignment', 'direction', 'colors', 'format', 'indent', 'lists', 'links', 'elements', 'media', 'tables', 'fullscreen'],
		eldorado : ['save', 'copypaste', 'elfinder', 'undoredo', 'style', 'alignment', 'colors', 'format', 'indent', 'lists', 'links', 'elements', 'media', 'tables']
...
elRTE.prototype.selection = function(rte) {
	...
	this.restoreIERange = function() {
		navigator.userAgent.match(/msie|trident/i) && bm && this.getRangeAt().moveToBookmark(bm);
	}
...
elRTE.prototype.ui.prototype.buttons.formatblock = function(rte, name) {
	...
	var cmd = navigator.userAgent.match(/msie|trident/i) 
...
elRTE.prototype.ui.prototype.buttons.fullscreen = function(rte, name) {
	...
	this.command = function() {
		var w = $(window),
			...
			m = navigator.userAgent.match(/mozilla/i),
			b, h;
...
elRTE.prototype.ui.prototype.buttons.link = function(rte, name) {
	...
	this.command = function() {
		...
		if (!this.link) {
			sel = navigator.userAgent.match(/msie|trident/i) ? this.rte.selection.selected() : this.rte.selection.selected({wrap : false});
			if (sel.length) {
		...
		opts = {
			...
			close : function() {navigator.userAgent.match(/msie|trident/i) && self.rte.selection.restoreIERange(); },
			dialog : {
				width : 'auto',
				width : 430,
...
elRTE.prototype.ui.prototype.buttons.pastetext = function(rte, name) {
	...
	this.command = function() {
		navigator.userAgent.match(/msie|trident/i) && this.rte.selection.saveIERange();
		var opts = {
			submit : function(e, d) {
				e.stopPropagation();
				e.preventDefault();
	...
	this.paste = function() {
		...
		if (txt) {
			this.rte.history.add();
			navigator.userAgent.match(/msie|trident/i) && this.rte.selection.restoreIERange();
	...
	elRTE.prototype.ui.prototype.buttons.unlink = function(rte, name) {
		...
		this.command = function() {
			...
			if (!l) {
				...
				var sel = navigator.userAgent.match(/msie|trident/i) ? this.rte.selection.selected() : this.rte.selection.selected({wrap : false});
				if (sel.length) {
