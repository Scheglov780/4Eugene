 ...
 *     2) Register class paths to [CaptchaExtendedAction] and [CaptchaExtendedValidator], e.g. in
 *     components/controller.php: public function init(){
 *                // import class paths for captcha extended
 *                Yii::$classMap = array_merge( Yii::$classMap, array(
 *                    'CaptchaExtendedAction' =>
 *                    Yii::getPathOfAlias('ext.captchaExtended').DIRECTORY_SEPARATOR.'CaptchaExtendedAction.php',
 *                    'CaptchaExtendedValidator' =>
 *                    Yii::getPathOfAlias('ext.captchaExtended').DIRECTORY_SEPARATOR.'CaptchaExtendedValidator.php'
 *                ));
 *            }
 ...
 *                );
 *            }
 *       5) If needed, collect localized strings via CLI command "yiic message messages/config.php" and translate
 ...
 *          - If needed, set the dots density [0-100], the number of through lines [0-] or fillSections [0-], font and
 *          background colors
 *       7) Test & enjoy!
 */
class CaptchaExtendedAction extends CCaptchaAction
{
     ...
     */
    public $lines = 3; // dots density 0 - 100%
    /**
    ...
    protected function getCodeWords()
    {
        ...
        if (!file_exists($this->fileWords)) {
            throw new CHttpException(500, Yii::t('main', 'File not found in "{path}"', ['{path}' => $this->fileWords]));
        }
    ...
    protected function renderImage($code)
    {
        ...
        $foreColor =
          imagecolorallocate($image, mt_rand($r - 50, $r + 50), mt_rand($g - 50, $g + 50), mt_rand($b - 50, $b + 50));
    ...
    public function run()
    {
        ...
        if (!extension_loaded('mbstring')) {
            throw new CHttpException(500, Yii::t('main', 'Missing extension "{ext}"', ['{ext}' => 'mbstring']));
        }
    ...
    public function validate($input, $caseSensitive)
    {
        ...
        if (empty($_POST['ajax'])) {
            ...
            /* 
			Yii::app()->session->add($name, $session[$name] + 1);
			*/
