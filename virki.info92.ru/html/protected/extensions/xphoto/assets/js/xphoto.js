...
var XPhoto = XPhotoClass({
    ...
    createButtons: function () {
        var buttons = '';
        if (this.options.capture === true) {
            buttons += '<button type="button" class="btn btn-success xphoto-button-camera"><i class="icon-camera"></i></button>'
                + '<button type="button" class="btn btn-primary xphoto-button-snap" style="display:none;"><i class="icon-check"></i></button>';
        }
        if (this.options.upload === true) {
            buttons += '<button type="button" class="btn btn-default xphoto-button-upload"><i class="icon-upload-alt"></i></button>';
        }
    ...
    buildModal: function () {
        ...
        var modal = '<div id="xphoto-modal" class="modal fade">'
            ...
            + '<h4 class="modal-title">Video capture</h4>'
            + '</div>'
            + '<div class="modal-body">'
            + '<div class="xphoto-video-container">'
            + '<video id="xphoto-modal-video" width="90%"></video>'
            + '</div>'
            + '<div class="xphoto-modal-buttons">'
            + '<button type="button" class="xphoto-modal-button-snap btn btn-primary btn-lg"><i class="icon-facetime-video"></i></button>'
            + '</div>'
            + '</div>'
            + '</div>'
            + '</div>'
    ...
    error: function (error) {
        if (error === 'NO_DEVICES_FOUND') {
            error = 'Camera not found! Check the cable.';
        }
