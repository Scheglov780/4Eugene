 ...
 * @since   1.0
 */
class DSGDbHttpSession extends CHttpSession
{
    /**
     ...
     */
    public $sessionTableName = 'dsg_sessions';
    ...
    protected function createSessionTable($db, $tableName)
    {
        ...
        $sql = "CREATE TABLE IF NOT EXISTS `{$this->sessionTableName}` (
  \"id\" char(32) CHARACTER SET ascii NOT NULL,
  \"expire\" datetime DEFAULT NULL,
  \"data\" {$blob},
  PRIMARY KEY (\"id\"),
  KEY \"idx_expire\" (\"expire\")
)  {$engine} COMMENT='php-сессии';";
    ...
    public function destroySession($id)
    {
        $this->getDbConnection()->createCommand()
          ->delete($this->sessionTableName, 'id=:id', [':id' => $id]);
        return true;
    }
    ...
    {
        $this->getDbConnection()->createCommand()
          ->delete($this->sessionTableName, 'expire<:expire', [':expire' => date("Y-m-d H:i:s", time())]);
        return true;
    }
    ...
    public function readSession($id)
    {
        ...
        $data = $db->createCommand()
          ...
          ->where('expire>:expire AND id=:id', [':expire' => date("Y-m-d H:i:s", time()), ':id' => $id])
          ->queryScalar();
...
//==========================
        ...
        if ($row !== false) {
            ...
            $db->createCommand()->insert($this->sessionTableName, [
              'id'     => $newID,
              'expire' => date("Y-m-d H:i:s", time() + $this->getTimeout()),
              'data'   => '',
            ]);
        }
    }
