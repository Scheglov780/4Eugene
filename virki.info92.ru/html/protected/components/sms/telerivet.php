    ...
    function doRequest($method, $path, $params = null)
    {
        ...
        if ($network_error) {
            ...
            if (isset($response['error'])) {
                ...
                switch ($error_code) {
                    case 'invalid_param':
                        throw new Telerivet_InvalidParameterException(
                ...
                if ($response) {
                    ...
                    if (json_last_error() != JSON_ERROR_NONE || $response_json === '') {
                        $info = curl_getinfo($curl);
                        $http_code = $info['http_code'];
                        throw new Telerivet_IOException(
                          "Unexpected response from Telerivet API (HTTP {$http_code}): {$response_json}"
                        );
                    } else {
                        return $response;
                    }
