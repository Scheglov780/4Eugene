...
class Telerivet_DataTable extends Telerivet_Entity
{
    ...
    {
        return new Telerivet_DataRow(
          $this->_api,
          $this->_api->doRequest("POST", "{$this->getBaseApiPath()}/rows", $options)
        );
    }
    ...
    {
        return new Telerivet_DataRow(
          $this->_api, $this->_api->doRequest("GET", "{$this->getBaseApiPath()}/rows/{$id}")
        );
    }
    ...
    function initRowById($id)
    {
        return new Telerivet_DataRow(
          $this->_api,
          ['project_id' => $this->project_id, 'table_id' => $this->id, 'id' => $id],
          false
        );
    }
