 ...
 * Read-only
 * - status_counts (associative array)
 ...
 * Read-only
 */
class Telerivet_Broadcast extends Telerivet_Entity
{
    /**
     * $broadcast->cancel()
    ...
    function cancel()
    {
        return new Telerivet_Broadcast($this->_api, $this->_api->doRequest("POST", "{$this->getBaseApiPath()}/cancel"));
    }

    function getBaseApiPath()
    {
        return "/projects/{$this->project_id}/broadcasts/{$this->id}";
    }
}
