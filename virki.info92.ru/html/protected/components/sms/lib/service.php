...
class Telerivet_Service extends Telerivet_Entity
{
    ...
    {
        return new Telerivet_ContactServiceState(
          $this->_api,
          $this->_api->doRequest('GET', $this->getBaseApiPath() . '/states/' . $contact->id)
        );
    }
    ...
    function resetContactState($contact)
    {
        return new Telerivet_ContactServiceState(
          $this->_api,
          $this->_api->doRequest('DELETE', $this->getBaseApiPath() . '/states/' . $contact->id)
        );
    }
    ...
    function setContactState($contact, $options)
    {
        return new Telerivet_ContactServiceState(
          $this->_api,
          $this->_api->doRequest('POST', $this->getBaseApiPath() . '/states/' . $contact->id, $options)
        );
    }
}
