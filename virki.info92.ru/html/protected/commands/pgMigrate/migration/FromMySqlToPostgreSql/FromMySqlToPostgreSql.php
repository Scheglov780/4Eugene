...
class FromMySqlToPostgreSql
{
    ...
    private function connect()
    {
        ...
        if (empty($this->pgsql)) {
            ...
            //Alexy $this->pgsql->setAttribute(\PDO::ATTR_EMULATE_PREPARES, false);
        ...
        try {
            ...
            foreach ($arrColumns as $arrColumn) {
                if ('auto_increment' == $arrColumn['Extra']) {
                    ...
                    $sql = 'ALTER TABLE "' . $this->strSchema . '"."' . strtolower($strTableName) . '" '
                      . 'ALTER COLUMN "' . strtolower($arrColumn['Field']) . '" '
                    ...
                    $sql = 'ALTER SEQUENCE "' .
                      ...
                      $this->strSchema .
                      '"."' .
                      strtolower($strTableName) .
                      '"."' .
                      strtolower($arrColumn['Field']) .
                      '";';
                    ...
                    $sql = 'SELECT SETVAL(\'"' . $this->strSchema . '"."' . $strSeqName . '"\', '
                      . '(SELECT MAX("' . strtolower($arrColumn['Field']) . '") FROM "'
                      . $this->strSchema . '"."' . strtolower($strTableName) . '"));';
                ...
                if ($boolSequenceCreated) {
                    unset($arrColumn);
                    $this->log(
                      "\t" . '-- Sequence "' . $this->strSchema . '"."' . $strSeqName . '" was created...' . PHP_EOL
                    );
                    break;
                }
    ...
    private function createTable($strTableName)
    {
        ...
        try {
            ...
            if (!$this->isDataOnly) {
                $sql = 'DROP TABLE IF EXISTS "' . $this->strSchema . '"."' . strtolower($strTableName) . '" CASCADE;';
                $this->pgsql->exec($sql);
            }
            ...
            $strSqlCreateTable = 'CREATE TABLE "' . $this->strSchema . '"."' . strtolower($strTableName) . '"(';
            $columnComments = '';
            foreach ($arrColumns as $arrColumn) {
                $strSqlCreateTable .= '"' .
                  strtolower($arrColumn['Field']) .
                  '" ' .
                  \MapDataTypes::map($arrColumn['Type']) .
                  ',';
            ...
            $strSqlCreateTable = $strSqlCreateTable . '
            COMMENT ON TABLE "' . $this->strSchema . '"."' . strtolower($strTableName) . '" IS \'' . $tableComment . '\';
            ' . $columnComments;

            //

            $stmt = $this->pgsql->exec($strSqlCreateTable);
            $boolRetVal = true;
    ...
    private function createView($strViewName)
    {
        ...
        try {
            ...
            $this->log(
              PHP_EOL . "\t" . '-- View: "' . $this->strSchema . '"."' . $strViewName . '" is created...' . PHP_EOL
            );
            ...
            $strMsg =
              $boolViewsErrorsDirectoryExists &&
              file_exists($this->strViewsErrorsDirectoryPath . '/' . $strViewName . '.sql')
                ?
                __METHOD__ .
                PHP_EOL .
                "\t" .
                ...
                PHP_EOL .
                "\t" .
                '-- You can find view definition at "logs_directory/not_created_views/' .
                $strViewName .
                '.sql"'
                .
                PHP_EOL .
                "\t" .
                '-- You can try to fix view definition script and run it manually.'
                :
                __METHOD__ .
                PHP_EOL .
                "\t" .
            ...
            $this->log(
              PHP_EOL . "\t" . '-- Cannot create view "' . $this->strSchema . '"."' . $strViewName . '" ' . PHP_EOL
            );
            $this->generateError($e, $strMsg, $sql);
    ...
    private function generateReport()
    {
        ...
        foreach ($this->arrSummaryReport as $arrReport) {
            ...
            $intLargestTableTitle =
              $intLargestTableTitle > $intTableTitleLength ? $intLargestTableTitle : $intTableTitleLength;
            $intLargestRecordsTitle =
              $intLargestRecordsTitle > $intRecordsTitleLength ? $intLargestRecordsTitle : $intRecordsTitleLength;
            $intLargestTimeTitle =
              $intLargestTimeTitle > $intTimeTitleLength ? $intLargestTimeTitle : $intTimeTitleLength;
    ...
    private function populateTableByPrepStmtWorker(
      ...
      $intRowsInserted
        ...
        try {
            ...
            $strColumns = '(';
            $strValues = '(';
            $strInsert = 'INSERT INTO "' . $this->strSchema . '"."' . strtolower($strTableName) . '" ';
            $intColumnIndex = 0;

            foreach ($arrRows[$intStartInsertionsFromIndex] as $strColumn => $value) {
                $strColumns .= '"' . strtolower($strColumn) . '",';
    ...
    private function populateTableWorker(
      ...
      $intForNowInserted
        ...
        try {
            ...
            if (file_exists($strAddrCsv)) {
                $sql = "COPY \"" . $this->strSchema . "\".\"" . strtolower($strTableName) . "\" FROM '" . $strAddrCsv . "' DELIMITER ',' 
                NULL 'NULL'
                CSV
                 -- QUOTE '\"'
                 -- ESCAPE '\\'
    ...
    private function processDefault($strTableName, array $arrColumns)
    {
        ...
        foreach ($arrColumns as $arrColumn) {
            ...
            try {
                $this->connect();
                $sql = 'ALTER TABLE "' . $this->strSchema . '"."' . strtolower($strTableName) . '" '
                  . 'ALTER COLUMN "' . strtolower($arrColumn['Field']) . '" SET DEFAULT ';
                ...
                if ($stmt === false) {
                    $this->log(
                      "\t" . '-- Cannot define the default value for column "' . $arrColumn['Field'] . '"...' . PHP_EOL
                    );
                } else {
                    $this->log(
                      "\t" . '-- The default value for column "' . $arrColumn['Field'] . '" has defined...' . PHP_EOL
                    );
                }
            } catch (\PDOException $e) {
    ...
    {
        $this->log(
          PHP_EOL . "\t" . '-- Set "ENUMs" for table "' . $this->strSchema . '"."' . $strTableName . '"...' . PHP_EOL
        );
        $sql = '';
            ...
            try {
                ...
                if (false !== $parenthesesFirstOccurrence) {
                    ...
                    if ('enum' == $arrType[0]) {
                        // $arrType[1] ends with ')'.
                        $sql = 'ALTER TABLE "' . $this->strSchema . '"."' . strtolower($strTableName) . '" '
                          . 'ADD CHECK ("' . strtolower($arrColumn['Field']) . '" IN (' . $arrType[1] . ');';
    ...
    private function processForeignKey($strTableName)
    {
        ...
        try {
            $this->log(
              "\t" . '-- Search foreign key for table "' . $this->strSchema . '"."' . $strTableName . '"...' . PHP_EOL
            );
            $this->connect();
            ...
            foreach ($arrConstraints as $arrRows) {
                ...
                $strDeleteRule = '';
                $strUpdateRule = '';
                $sql = 'ALTER TABLE "' . $this->strSchema . '"."' . strtolower($strTableName) . '" ADD FOREIGN KEY (';

                foreach ($arrRows as $arrRow) {
                    $strRefTbName = $arrRow['referenced_table_name'];
                    $strUpdateRule = $arrRow['update_rule'];
                    $strDeleteRule = $arrRow['delete_rule'];
                    $arrFKs[] = '"' . strtolower($arrRow['column_name']) . '"';
                    $arrPKs[] = '"' . strtolower($arrRow['referenced_column_name']) . '"';
                    unset($arrRow);
                }
    ...
    private function processIndexAndKey($strTableName, array $arrColumns)
    {
        ...
        try {
            ...
            foreach ($arrIndices as $arrIndex) {
                if ($arrIndex['Index_type'] == 'FULLTEXT') {
                    continue;
                }
                if (isset($arrPgIndices[$arrIndex['Key_name']])) {
                    $arrPgIndices[$arrIndex['Key_name']]['column_name'][] =
                      '"' . strtolower($arrIndex['Column_name']) . '"';
                } else {
                    if (false && 0 == $arrIndex['Non_unique']) {
                        $indexType = 'HASH';
                    } else {
                        $indexType = ($arrIndex['Index_type'] === 'SPATIAL' ? 'GIST' : $arrIndex['Index_type']);
                    }
                    $arrPgIndices[$arrIndex['Key_name']] = [
                      'is_unique'   => (0 == $arrIndex['Non_unique'] ? true : false),
                      'column_name' => ['"' . strtolower($arrIndex['Column_name']) . '"'],
                      'Index_type'  => ' USING ' . $indexType,
                    ];
                }
                unset($arrIndex);
            }
        ...
        foreach ($arrColumns as $arrColumn) {
            try {
                ...
                if ('no' == strtolower($arrColumn['Null'])) {
                    $sql = 'ALTER TABLE "' . $this->strSchema . '"."' . strtolower($strTableName)
                      . '" ALTER COLUMN "' . strtolower($arrColumn['Field']) . '" SET NOT NULL;';
    ...
    private function runVacuumFullAndAnalyze($strTableName)
    {
        ...
        try {
            $this->connect();
            $sql = 'VACUUM (FULL, ANALYZE) "' . $this->strSchema . '"."' . strtolower($strTableName) . '";';
            $stmt = $this->pgsql->query($sql);
        ...
        switch ($strValue) {
            ...
            default:
                if (is_null($strValue)) {
                    ...
                    $val = substr($this->pgsql->quote($strValue), 1, -1);
                }
                return $val;
        }
    }
    ...
    private function setTableConstraints($strTableName)
    {
        $this->log(
          "\t" .
          '-- Trying to set table constraints for "' .
          $this->strSchema .
          '"."' .
          $strTableName .
          '"...' .
